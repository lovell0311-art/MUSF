...
namespace ET.Analyzer
{
    ...
    public class EntityFiledAccessAnalyzer: DiagnosticAnalyzer
    {
        ...
        private const string MessageFormat = "实体: {0} 字段: {1} 只能在实体类生命周期组件或友元类(含有FriendOfAttribute)中访问。(不是自己的对象，不要使用FriendOfAttribute标签。让对象维护者提供接口)";
        ...
        private void AnalyzeMemberAccessExpression(SyntaxNodeAnalysisContext context)
        {
            ...
            if (!filedSymbol.ContainingType.HasAttribute(Definition.PrivateObjectAttribute))
            {
                return;
            }
            ...
            // 实体基类忽略处理
            if (accessFieldClassSymbol.HasAttribute(Definition.PrivateObjectAttribute))
            {
                return;
            }
