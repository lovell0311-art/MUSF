...
{
    public static class TaskInSyncMethodAnalyzerRule
    {
        private const string Title = "Task方法调用在非异步方法体内使用错误";

        private const string MessageFormat = "方法: {0} 在非异步方法体内使用时需要添加.Coroutine()后缀";

        private const string Description = "Task方法调用在非异步方法体内使用错误.";
    ...
    public static class TaskInAsyncMethodAnalyzerRule
    {
        private const string Title = "Task方法调用在异步方法体内使用错误";

        private const string MessageFormat = "方法: {0} 在异步方法体内使用时需要添加await前缀 或 .Coroutine()后缀";

        private const string Description = "Task方法调用在异步方法体内使用错误.";
    ...
    public static class AsyncVoidAnalyzerRule
    {
        private const string Title = "实现了一个不安全的异步方法";

        private const string MessageFormat = "禁止创建 async void 方法: {0}, 请改为 async Task";

        private const string Description = "禁止创建 async void 方法.";

        public static readonly DiagnosticDescriptor Rule =
                new DiagnosticDescriptor(DiagnosticIds.AsyncVoidAnalyzerRuleId,
                    Title,
                    MessageFormat,
                    DiagnosticCategories.Hotfix,
                    DiagnosticSeverity.Error,
