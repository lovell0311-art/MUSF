...
namespace ETHotfix.Robot
{
    ...
    public class G2C_UnitEquipUnload_noticeHandler : AMHandler<G2C_UnitEquipUnload_notice>
    {
        protected async override Task<bool> BeforeCodeAsync(Session session, G2C_UnitEquipUnload_notice message)
        {
        ...
        protected async override Task<bool> CodeAsync(Session session, G2C_UnitEquipUnload_notice message)
        {
            ...
            equipment.UnloadEquipItem((EquipPosition)message.EquipPosition);

            targetUnit.GetComponent<ObjectWait>().Notify(new Wait_Unit_UnloadEquipItem());
            return false;
        }
    }
}
