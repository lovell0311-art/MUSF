...
namespace ETHotfix.Robot
{
    ...
    public class G2C_OpenSkillGroup_noticeHandler : AMHandler<G2C_OpenSkillGroup_notice>
    {
        protected async override Task<bool> BeforeCodeAsync(Session session, G2C_OpenSkillGroup_notice message)
        {
        ...
        protected async override Task<bool> CodeAsync(Session session, G2C_OpenSkillGroup_notice message)
        {
            ...
            foreach(int skillId in message.SkillIds)
            {
                RobotSkill skill = RobotSkillFactory.Create(clientScene, skillId);
                if (skill == null)
                {
                    Log.Warning($"[{clientScene.Name}] 技能配置不存在，skillId={skillId}");
                    continue;
                }
