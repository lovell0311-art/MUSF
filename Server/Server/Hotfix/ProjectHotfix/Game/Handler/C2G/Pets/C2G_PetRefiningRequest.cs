...
namespace ETHotfix
{
    ...
    public class C2G_PetRefiningRequestHandler : AMActorRpcHandler<C2G_PetRefiningRequest, G2C_PetRefiningResponse>
    {
        protected override async Task<bool> BeforeCodeAsync(Session b_Connect, C2G_PetRefiningRequest b_Request, G2C_PetRefiningResponse b_Response, Action<IMessage> b_Reply)
        {
        ...
        protected override async Task<bool> Run(C2G_PetRefiningRequest b_Request, G2C_PetRefiningResponse b_Response, Action<IMessage> b_Reply)
        {
            ...
            if (pets != null && pets.dBPetsData.PetsId == b_Request.PetsID)
            {
                ...
                Dictionary<int, int> EMInfo = new Dictionary<int, int>() { { 280003, 1 }, { 280004, 1 } };
                if (EMInfo != null)
                {
                    ...
                    foreach (var item in petsItem)
                    {
                        ...
                        if (ItemList != null)
                        {
                            ...
                            foreach (var item2 in ItemList)
                            {
                                ...
                                {
                                    if (!knapsack.UseItem(item2.Value, "宠物精炼", Cnt))
                                    {
                                        //代码到这里表示代码出了异常
                                        Log.Error($"宠物精炼，使用道具失败GamePlayer:{mPlayer.GameUserId}");
                                    }
                                    Cnt = 0;
                                    break;
                                }
                                else
                                {
                                    ...
                                    if (!knapsack.UseItem(item2.Value, "宠物精炼", item2.Value.GetProp(EItemValue.Quantity)))
                                    {
                                        //代码到这里表示代码出了异常
                                        Log.Error($"宠物精炼，使用道具失败GamePlayer:{mPlayer.GameUserId}");
                                    }
                    ...
                    pets.dBPetsData.ExcellentId.Clear();
                    ...
                    if (excAttrEntryManager.TryGetPetsAttrEntry(out var selector))
                    {
                        if (excAttrEntryManager.PetsExcAttrEntryCount.TryGetValue(out int count))
                        {
                            ...
                            do
                            {
                                ...
                                {
                                    pets.dBPetsData.ExcellentId.Add(entryId);
                                }
                                else
                                {
                                    // 词条取空了
                    ...
                    pets.dBPetsData.Excellent = Help_JsonSerializeHelper.Serialize(pets.dBPetsData.ExcellentId);
