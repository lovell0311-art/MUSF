...
namespace ETHotfix
{
    ...
    public class C2G_PetsEnhanceRequestHandler : AMActorRpcHandler<C2G_PetsEnhanceRequest, G2C_PetsEnhanceResponse>
    {
        protected override async Task<bool> BeforeCodeAsync(Session b_Connect, C2G_PetsEnhanceRequest b_Request, G2C_PetsEnhanceResponse b_Response, Action<IMessage> b_Reply)
        {
        ...
        protected override async Task<bool> Run(C2G_PetsEnhanceRequest b_Request, G2C_PetsEnhanceResponse b_Response, Action<IMessage> b_Reply)
        {
            ...
            if (pets != null && pets.dBPetsData.PetsId == b_Request.PetsID)
            {
                ...
                int PetRate = EnhanceMaterials[pets.dBPetsData.EnhanceLv + 1].SuccessRate;
                if (EMInfo != null)
                {
                    ...
                    foreach (var item in petsItem)
                    {
                        ...
                        if (ItemList != null)
                        {
                            ...
                            foreach (var item2 in ItemList)
                            {
                                ...
                                {
                                    if (!knapsack.UseItem(item2.Value, "宠物强化", Cnt))
                                    {
                                        //代码到这里表示代码出了异常
                                        Log.Error($"宠物强化，使用道具失败GamePlayer:{mPlayer.GameUserId}");
                                    }
                                    Cnt = 0;
                                    break;
                                }
                                else
                                {
                                    ...
                                    if (!knapsack.UseItem(item2.Value, "宠物强化", item2.Value.GetProp(EItemValue.Quantity)))
                                    {
                                        //代码到这里表示代码出了异常
                                        Log.Error($"宠物强化，使用道具失败GamePlayer:{mPlayer.GameUserId}");
                                    }
                    ...
                    if (PetRate >= Rate)//成功
                    {
                        pets.dBPetsData.EnhanceLv++;
                    }
                    else//失败
                    {
                        if (pets.dBPetsData.EnhanceLv >= 7)
                            pets.dBPetsData.EnhanceLv = 0;
                        else
                            pets.dBPetsData.EnhanceLv--;
                        b_Response.Error = Root.MainFactory.GetCustomComponent<LanguageComponent>().GetMessageId(1611);
                    }
