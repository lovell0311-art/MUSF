...
namespace ETHotfix
{
    ...
    public class C2G_ShouQCreateAnOrderHandler : AMActorRpcHandler<C2G_ShouQCreateAnOrder, G2C_ShouQCreateAnOrder>
    {
        protected override async Task<bool> BeforeCodeAsync(Session b_Connect, C2G_ShouQCreateAnOrder b_Request, G2C_ShouQCreateAnOrder b_Response, Action<IMessage> b_Reply)
        {
        ...
        protected override async Task<bool> Run(C2G_ShouQCreateAnOrder b_Request, G2C_ShouQCreateAnOrder b_Response, Action<IMessage> b_Reply)
        {
            //b_Reply(b_Response);
            //return true;
            ...
            DBPlayerPayOrderInfo mPlayerPayOrderInfo = new DBPlayerPayOrderInfo
            {
                ...
                TradePlatform = TradePlatform.ShouQ,
                App_Ordef_id = IdGeneraterNew.Instance.GenerateUnitId(mPlayer.GameAreaId),
                Gid = 112,
                Uid = long.Parse(dbLoginInfo.ShouQAccountNumber),
                GUid = mPlayer.UserId,
           ...
           //"http://120.24.45.40:15080/api/Pay/PayInfoShouQ";
            b_Response.PayRmb = Value;
            b_Response.ExInfo = mPlayer.SourceGameAreaId.ToString();
            b_Reply(b_Response);
            return true;
        }
    }
