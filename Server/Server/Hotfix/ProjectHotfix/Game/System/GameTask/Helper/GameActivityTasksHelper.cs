...
namespace ETHotfix
{
    ...
    /// 活动任务
    /// </summary>
    public static class GameActivityTasksHelper
    {
        public static void Init(GameTasksComponent tasksCom)
        {
            if(tasksCom.data.ActivityTasks.Count == 0)
            {
                // 初始化任务
        ...
        {
            if (beforeTaskConfigId != 0) return;    // 只自动领取第一个任务
            ...
            foreach (var taskConfigId in taskConfigIdList)
            {
                ...
                if (conf.TaskType != EGameTaskType.Activity)
                {
                    if(beforeTaskConfigId != 0) Log.Error($"活动任务，不能切换到其他任务。taskId={conf.ConfigId},taskType={conf.TaskType}");
                    continue;
                }
        ...
        {
            foreach(var gameTask in tasksCom.data.ActivityTasks.Values)
            {
                ...
                if (gameTask.Config.CanReceive(tasksCom.Parent.GetCustomComponent<GamePlayer>()))
                {
                    ...
                    Log.Info($"[活动任务] 自动领取任务 taskId = {gameTask.ConfigId} state = {gameTask.TaskState}");
                }
            }
        }
        ...
        {
            if(tasksCom.data.ActivityTasks.ContainsKey(gameTask.ConfigId))
            {
                // 已经领取了这个任务
                return false;
            }
            tasksCom.data.ActivityTasks.Add(gameTask.ConfigId, gameTask);
            ...
            Log.Info($"[活动任务] 添加任务 taskId = {gameTask.ConfigId} state = {gameTask.TaskState}");
            return true;
        }

        public static void TriggerTaskAction(GameTasksComponent tasksCom, int taskConfigId, int progressId, int value)
        {
            if(!tasksCom.data.ActivityTasks.TryGetValue(taskConfigId,out var gameTask))
            {
                return;
            }
            ...
            // 通知玩家，任务变动
            gameTask.NotifyChanged(tasksCom.Parent);
            Log.Info($"[活动任务] 触发行为 taskId = {gameTask.ConfigId} progressId = {progressId} value = {value} state = {gameTask.TaskState}");
        }
    ...
    public class GamePlayerLevelUp_ActivityTasksHelper : ITEventMethodOnRun<ETModel.EventType.GamePlayerLevelUp>
    {
        public void OnRun(ETModel.EventType.GamePlayerLevelUp args)
        {
            ...
            GameActivityTasksHelper.UpdateStatelessTasks(gameTasksCom);
        }

    }

    [EventMethod("GamePlayerCareerChangeComplete")]
    public class GamePlayerCareerChangeComplete_ActivityTasksHelper : ITEventMethodOnRun<ETModel.EventType.GamePlayerCareerChangeComplete>
    {
