...
namespace ETHotfix
{
    ...
    /// 委托任务
    /// </summary>
    public static class GameEntrustTasksHelper
    {
        ...
        {
            tasksCom.data.EntrustTask = gameTask;
            ...
            Log.Info($"[委托任务] 添加任务 taskId = {gameTask.ConfigId} state = {gameTask.TaskState}");
            return true;
        }

        public static void TriggerTaskAction(GameTasksComponent tasksCom, int taskConfigId, int progressId, int value)
        {
            if (tasksCom.data.EntrustTask == null) return;
            tasksCom.data.EntrustTask.UpdateProgress(progressId, value);
            tasksCom.data.EntrustTask.TryCompleteTask();

            tasksCom.SaveDB();
            // 通知玩家，任务变动
            tasksCom.data.EntrustTask.NotifyChanged(tasksCom.Parent);
            Log.Info($"[委托任务] 触发行为 taskId = {tasksCom.data.EntrustTask.ConfigId} progressId = {progressId} value = {value} state = {tasksCom.data.EntrustTask.TaskState}");
        }
