...
namespace ETHotfix
{
    ...
    public class ItemRandAddExcAttr_3_6 : IItemCustomAttrMethodHandler
    {
        public void Run(Item item)
        {
            if (!item.CanHaveExcellentOption())
            {
                throw new ItemNotSupportAttrException($"物品不支持添加卓越属性，item.ConfigId={item.ConfigID},ItemCustomAttrMethod='ItemRandAddExcAttr_3_6'");
            }
    ...
    /// </summary>
    [ItemCustomAttrMethod]
    public class ItemRandAddExcAttr_1 : IItemCustomAttrMethodHandler
    {
        public void Run(Item item)
        {
            if (!item.CanHaveExcellentOption())
