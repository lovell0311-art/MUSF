...
namespace ETHotfix
{
    ...
    public class ItemAddSocketHoleCountFull : IItemCustomAttrMethodHandler
    {
        public void Run(Item item)
        {
            if(!item.CanHaveEnableSocket())
            {
                throw new ItemNotSupportAttrException($"物品不支持添加镶嵌属性，item.ConfigId={item.ConfigID},ItemCustomAttrMethod='ItemAddSocketHoleCountFull'");
            }
