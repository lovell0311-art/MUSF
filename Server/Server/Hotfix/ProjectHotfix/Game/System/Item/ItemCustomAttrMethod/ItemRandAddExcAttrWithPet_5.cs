...
{
    /// <summary>
    /// 随机添加卓越属性 用宠物的方式 最低 5条
    /// </summary>
    [ItemCustomAttrMethod]
    public class ItemRandAddExcAttrWithPet_5 : IItemCustomAttrMethodHandler
    {
        public void Run(Item item)
        {
            if (!item.CanHaveExcellentOption())
            {
                throw new ItemNotSupportAttrException($"物品不支持添加卓越属性，item.ConfigId={item.ConfigID},ItemCustomAttrMethod='ItemRandAddExcAttrWithPet_5'");
            }
