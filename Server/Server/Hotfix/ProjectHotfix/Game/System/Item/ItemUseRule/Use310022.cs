...
namespace ETHotfix
{
    ...
    /// 体力卷轴
    /// </summary>
    [ItemUseRule(typeof(Use310022))]
    public class Use310022 : C_ItemUseRule<Player, Item, IResponse>
    {
        public override async Task Run(Player b_Player, Item b_Item, IResponse b_Response)
        {
            ...
            Func<CombatSource.BattleSyncTimerTask> mCreateFunc = () =>
            {
                ...
                mBattleSyncTimer.DisposeAction = (b_CombatSource, b_BattleComponent, b_TimerTask) =>
                {
                    ...
                    if (b_CombatSource.HealthStatsDic.TryGetValue(E_BattleSkillStats.Use310022, out var hp_Curse))
                    {
            ...
            mCurrentTemp.AddHealthState(E_BattleSkillStats.Use310022, b_Item.ConfigData.Value, b_Item.ConfigData.Value2, 0, mCreateFunc, mBattleComponent);
            mCurrentTemp.UpdateHealthState();

            mCurrentTemp.Data.DBBufflist[(int)E_BattleSkillStats.Use310022] = mBattleComponent.CurrentTimeTick + b_Item.ConfigData.Value2;
