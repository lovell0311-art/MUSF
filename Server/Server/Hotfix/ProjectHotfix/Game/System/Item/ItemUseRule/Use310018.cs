...
namespace ETHotfix
{
    ...
    /// 加速卷轴
    /// </summary>
    [ItemUseRule(typeof(Use310018))]
    public class Use310018 : C_ItemUseRule<Player, Item, IResponse>
    {
        public override async Task Run(Player b_Player, Item b_Item, IResponse b_Response)
        {
            ...
            mCurrentTemp.AddHealthState(E_BattleSkillStats.Use310018, b_Item.ConfigData.Value, b_Item.ConfigData.Value2, 0, mCreateFunc, mBattleComponent);
            mCurrentTemp.UpdateHealthState();

            mCurrentTemp.Data.DBBufflist[(int)E_BattleSkillStats.Use310018] = mBattleComponent.CurrentTimeTick + b_Item.ConfigData.Value2;
