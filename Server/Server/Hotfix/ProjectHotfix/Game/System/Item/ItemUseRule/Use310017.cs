...
namespace ETHotfix
{
    ...
    /// 精华治疗药水
    /// </summary>
    [ItemUseRule(typeof(Use310017))]
    public class Use310017 : C_ItemUseRule<Player, Item, IResponse>
    {
        public override async Task Run(Player b_Player, Item b_Item, IResponse b_Response)
        {
            ...
            int maxHP = mGamePlayer.GetNumerial(E_GameProperty.PROP_MP_MAX);
            
            if (mGamePlayer.UnitData.Mp < maxHP)
            {
                mGamePlayer.UnitData.Mp = maxHP;
            }
            //保存数据库
