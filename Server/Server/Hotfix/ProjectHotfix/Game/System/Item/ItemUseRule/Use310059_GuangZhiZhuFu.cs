...
namespace ETHotfix
{
    ...
    /// 光之祝福（高级）
    /// </summary>
    [ItemUseRule(typeof(Use310059))]
    public class Use310059 : C_ItemUseRule<Player, Item, IResponse>
    {
        public override async Task Run(Player b_Player, Item b_Item, IResponse b_Response)
        {
            ...
            mCurrentTemp.AddHealthState(E_BattleSkillStats.UseGuangZhiZhuFu310059, b_Item.ConfigData.Value, b_Item.ConfigData.Value2, 0, mCreateFunc, mBattleComponent);
            mCurrentTemp.UpdateHealthState();

            mCurrentTemp.Data.DBBufflist[(int)E_BattleSkillStats.UseGuangZhiZhuFu310059] = mBattleComponent.CurrentTimeTick + b_Item.ConfigData.Value2;
