...
namespace ETHotfix
{
    [ItemUseRule(typeof(Use310140))]
    public class Use310140 : C_ItemUseRule<Player, Item, IResponse>
    {
        public override async Task Run(Player b_Player, Item b_Item, IResponse b_Response)
        {
            ...
            Func<CombatSource.BattleSyncTimerTask> mCreateFunc = () =>
            {
                ...
                mBattleSyncTimer.DisposeAction = (b_CombatSource, b_BattleComponent, b_TimerTask) =>
                {
                    ...
                    b_CombatSource.RemoveHealthState(E_BattleSkillStats.Use310140, b_BattleComponent);
                ...
                mBattleSyncTimer.SyncAction = (b_CombatSource, b_BattleComponent, b_TimerTask, b_TimeTick) =>
                {
                    ...
                    if (b_CombatSource.HealthStatsDic.TryGetValue(E_BattleSkillStats.Use310140, out var mCurrentHealthStats) == false)
                    {
            ...
            long Tiem = b_Item.ConfigData.Value2;
            if (mCurrentTemp.HealthStatsDic.ContainsKey(E_BattleSkillStats.Use310140))
                mCurrentTemp.AddHealthState(E_BattleSkillStats.Use310140, b_Item.ConfigData.Value, Tiem * 1000, 0, mCreateFunc, mBattleComponent, E_SkillStatsResetType.RESET_NUMBER);
            else
                mCurrentTemp.AddHealthState(E_BattleSkillStats.Use310140, b_Item.ConfigData.Value, Tiem * 1000, 0, mCreateFunc, mBattleComponent);

            mCurrentTemp.UpdateHealthState();

            mCurrentTemp.Data.DBBufflist.TryGetValue((int)E_BattleSkillStats.Use310140, out var Value);
            if (mBattleComponent.CurrentTimeTick > Value)
                Value = mBattleComponent.CurrentTimeTick;
            mCurrentTemp.Data.DBBufflist[(int)E_BattleSkillStats.Use310140] = Value + Tiem * 1000;
