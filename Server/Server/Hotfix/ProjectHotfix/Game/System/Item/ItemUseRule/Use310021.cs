...
namespace ETHotfix
{
    ...
    /// 魔力卷轴
    /// </summary>
    [ItemUseRule(typeof(Use310021))]
    public class Use310021 : C_ItemUseRule<Player, Item, IResponse>
    {
        public override async Task Run(Player b_Player, Item b_Item, IResponse b_Response)
        {
            ...
            mCurrentTemp.AddHealthState(E_BattleSkillStats.Use310021, b_Item.ConfigData.Value, b_Item.ConfigData.Value2, 0, mCreateFunc, mBattleComponent);
            mCurrentTemp.UpdateHealthState();

            mCurrentTemp.Data.DBBufflist[(int)E_BattleSkillStats.Use310021] = mBattleComponent.CurrentTimeTick + b_Item.ConfigData.Value2;
