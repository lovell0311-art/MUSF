...
namespace ETHotfix
{
    ...
    /// 攻速提升
    /// </summary>
    [ItemUseRule(typeof(Use310127))]
    public class Use310127 : C_ItemUseRule<Player, Item, IResponse>
    {
        public override async Task Run(Player b_Player, Item b_Item, IResponse b_Response)
        {
            ...
            Func<CombatSource.BattleSyncTimerTask> mCreateFunc = () =>
            {
                ...
                mBattleSyncTimer.DisposeAction = (b_CombatSource, b_BattleComponent, b_TimerTask) =>
                {
                    ...
                    if (b_CombatSource.HealthStatsDic.TryGetValue(E_BattleSkillStats.Use310127, out var hp_Curse))
                    {
            ...
            int AttackSpeed = mCurrentTemp.GetNumerialFunc(E_GameProperty.AttackSpeed);
            AttackSpeed = AttackSpeed * b_Item.ConfigData.Value/100; 
            if (mCurrentTemp.HealthStatsDic.ContainsKey(E_BattleSkillStats.Use310127))
            {
                mCurrentTemp.AddHealthState(E_BattleSkillStats.Use310127, AttackSpeed, b_Item.ConfigData.Value2, 0, mCreateFunc, mBattleComponent, E_SkillStatsResetType.RESET_NUMBER);
                mCurrentTemp.UpdateHealthState();
            }
            else
            {
                mCurrentTemp.AddHealthState(E_BattleSkillStats.Use310127, AttackSpeed, b_Item.ConfigData.Value2, 0, mCreateFunc, mBattleComponent);
                mCurrentTemp.UpdateHealthState();
                ...
                mPropertyData.Value = mCurrentTemp.GetNumerialFunc(E_GameProperty.AttackSpeed);
                mChangeValueMessage.Info.Add(mPropertyData);
                mPropertyData = new G2C_BattleKVData();

                mCurrentTemp.Player.Send(mChangeValueMessage);
            }
            mCurrentTemp.Data.DBBufflist[(int)E_BattleSkillStats.Use310127] = mBattleComponent.CurrentTimeTick + b_Item.ConfigData.Value2;
