...
namespace ETHotfix
{
    ...
    /// 伤害吸收提升
    /// </summary>
    [ItemUseRule(typeof(Use310121))]
    public class Use310121 : C_ItemUseRule<Player, Item, IResponse>
    {
        public override async Task Run(Player b_Player, Item b_Item, IResponse b_Response)
        {
            ...
            Func<CombatSource.BattleSyncTimerTask> mCreateFunc = () =>
            {
                ...
                mBattleSyncTimer.DisposeAction = (b_CombatSource, b_BattleComponent, b_TimerTask) =>
                {
                    ...
                    if (b_CombatSource.HealthStatsDic.TryGetValue(E_BattleSkillStats.Use310120, out var hp_Curse))
                    {
            ...
            if (mCurrentTemp.HealthStatsDic.ContainsKey(E_BattleSkillStats.Use310120))
            {
                mCurrentTemp.AddHealthState(E_BattleSkillStats.Use310120, b_Item.ConfigData.Value, b_Item.ConfigData.Value2, 0, mCreateFunc, mBattleComponent, E_SkillStatsResetType.RESET_NUMBER);
                mCurrentTemp.UpdateHealthState();
            }
            else
            {
                mCurrentTemp.AddHealthState(E_BattleSkillStats.Use310120, b_Item.ConfigData.Value, b_Item.ConfigData.Value2, 0, mCreateFunc, mBattleComponent);
                //if (mCurrentTemp.GamePropertyDic.ContainsKey(E_GameProperty.HurtValueAbsorbRate))
                //{
                //    mCurrentTemp.GamePropertyDic[E_GameProperty.HurtValueAbsorbRate] += b_Item.ConfigData.Value * 100;
                //}
                //else
                //{
                //    mCurrentTemp.GamePropertyDic.Add(E_GameProperty.HurtValueAbsorbRate, b_Item.ConfigData.Value * 100);
                //}
                mCurrentTemp.UpdateHealthState();
                ...
                mPropertyData.Value = mCurrentTemp.GetNumerialFunc(E_GameProperty.HurtValueAbsorbRate);
                mChangeValueMessage.Info.Add(mPropertyData);
                mPropertyData = new G2C_BattleKVData();

                mCurrentTemp.Player.Send(mChangeValueMessage);
            }
            mCurrentTemp.Data.DBBufflist[(int)E_BattleSkillStats.Use310120] = mBattleComponent.CurrentTimeTick + b_Item.ConfigData.Value2;
            ...
            Func<CombatSource.BattleSyncTimerTask> mCreateFunc = () =>
            {
                ...
                {
                    if (b_CombatSource.IsDisposeable) return;
                    b_CombatSource.HealthStatsDic.TryGetValue(E_BattleSkillStats.Use310120, out var varInfo);
                    if (varInfo == null) return;
                    var HurtValueAbsorbRate = b_CombatSource.GetNumerialFunc(E_GameProperty.HurtValueAbsorbRate);
                    var ValueI = HurtValueAbsorbRate - varInfo.CacheDatas[0].StrengthValue * 100;

                    (b_CombatSource as GamePlayer).GamePropertyDic[E_GameProperty.HurtValueAbsorbRate] = ValueI;
            ...
            mCurrentTemp.Data.DBBufflist.TryGetValue((int)E_BattleSkillStats.Use310120, out var Value);
            if (mBattleComponent.CurrentTimeTick > Value)
                Value = mBattleComponent.CurrentTimeTick;

            mCurrentTemp.Data.DBBufflist[(int)E_BattleSkillStats.Use310120] = Value + b_Item.ConfigData.Value;
            mCurrentTemp.Data.Serialize();
