...
namespace ETHotfix
{
    ...
    /// 生命提升
    /// </summary>
    [ItemUseRule(typeof(Use310131))]
    public class Use310131 : C_ItemUseRule<Player, Item, IResponse>
    {
        public override async Task Run(Player b_Player, Item b_Item, IResponse b_Response)
        {
            ...
            Func<CombatSource.BattleSyncTimerTask> mCreateFunc = () =>
            {
                ...
                mBattleSyncTimer.DisposeAction = (b_CombatSource, b_BattleComponent, b_TimerTask) =>
                {
                    ...
                    if (b_CombatSource.HealthStatsDic.TryGetValue(E_BattleSkillStats.Use310131, out var hp_Curse))
                    {
            ...
            if (mCurrentTemp.HealthStatsDic.ContainsKey(E_BattleSkillStats.Use310131))
            {
                mCurrentTemp.AddHealthState(E_BattleSkillStats.Use310131, b_Item.ConfigData.Value, b_Item.ConfigData.Value2, 0, mCreateFunc, mBattleComponent, E_SkillStatsResetType.RESET_NUMBER);
                mCurrentTemp.UpdateHealthState();
            }
            else
            {
                mCurrentTemp.AddHealthState(E_BattleSkillStats.Use310131, b_Item.ConfigData.Value, b_Item.ConfigData.Value2, 0, mCreateFunc, mBattleComponent);
                mCurrentTemp.UpdateHealthState();
                ...
                mPropertyData.Value = mCurrentTemp.GetNumerialFunc(E_GameProperty.PROP_MP_MAX);
                mChangeValueMessage.Info.Add(mPropertyData);
                mPropertyData = new G2C_BattleKVData();

                mCurrentTemp.Player.Send(mChangeValueMessage);
            }
            mCurrentTemp.Data.DBBufflist[(int)E_BattleSkillStats.Use310131] = mBattleComponent.CurrentTimeTick + b_Item.ConfigData.Value2;
