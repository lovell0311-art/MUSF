...
namespace ETHotfix
{
    ...
    /// 倍数经验
    /// </summary>
    [ItemUseRule(typeof(Use310157))]
    public class Use310157 : C_ItemUseRule<Player, Item, IResponse>
    {
        public override async Task Run(Player b_Player, Item b_Item, IResponse b_Response)
        {
            ...
            if (mCurrentTemp.Data.ReincarnateCnt >= 1)
            {
                b_Response.Error = 725;
                return;
            }

            if (mCurrentTemp.Data.Level >= 300)
            {
                b_Response.Error = 725;
                return;
            }

            if (mCurrentTemp.HealthStatsDic.ContainsKey(E_BattleSkillStats.Use310157))
            {
            ...
            Func<CombatSource.BattleSyncTimerTask> mCreateFunc = () =>
            {
                ...
                mBattleSyncTimer.SyncWaitTime = Tiem;
            ...
            mCurrentTemp.AddHealthState(E_BattleSkillStats.Use310157, b_Item.ConfigData.Value, b_Item.ConfigData.Value2, 0, mCreateFunc, mBattleComponent);

            mCurrentTemp.UpdateHealthState();

            mCurrentTemp.Data.DBBufflist.TryGetValue((int)E_BattleSkillStats.Use310157, out var Value);
            if (mBattleComponent.CurrentTimeTick > Value)
                Value = mBattleComponent.CurrentTimeTick;
            mCurrentTemp.Data.DBBufflist[(int)E_BattleSkillStats.Use310157] = Value + b_Item.ConfigData.Value2;
