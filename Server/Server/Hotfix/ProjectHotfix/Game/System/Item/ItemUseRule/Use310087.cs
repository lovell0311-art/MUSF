...
namespace ETHotfix
{
    ...
    /// 天使项链
    /// </summary>
    [ItemUseRule(typeof(Use310087))]
    public class Use310087 : C_ItemUseRule<Player, Item, IResponse>
    {
        ...
        public override async Task Run(Player b_Player, Item b_Item, IResponse b_Response)
        {
            ...
            int MobId = 0;
            if (100 <= Lv && Lv <= 200)
                MobId = 562;//enemyList = new List<int>() { 562, 563 };
            else if (201 <= Lv && Lv <= 300)
                MobId = 563;//enemyList = new List<int>() { 564, 565, 566 };
            else //if(301 <= Lv)
                MobId = 564;//enemyList = new List<int>() { 567, 568, 569 };
            ...
            (int, int) Id = (14008, 30);
            Enemy monster = CreateMonster(map, enemyJsonDic[MobId], PosX, PosY, b_Player.GameUserId,14, Id, b_Item.ItemUID);
            G2C_CangBaoTuPosUpdate_notice notice2 = new G2C_CangBaoTuPosUpdate_notice()
            {
                ...
                PosY = PosY,
                NpcConfigID = MobId,
                TreasureType = ETreasureType.XiaoTianShi,
                Id = b_Item.ItemUID,
            };
