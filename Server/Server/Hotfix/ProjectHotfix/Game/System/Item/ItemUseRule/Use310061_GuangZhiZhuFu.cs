...
namespace ETHotfix
{
    ...
    /// 光之祝福（中级）
    /// </summary>
    [ItemUseRule(typeof(Use310061))]
    public class Use310061 : C_ItemUseRule<Player, Item, IResponse>
    {
        public override async Task Run(Player b_Player, Item b_Item, IResponse b_Response)
        {
            ...
            // 使用道具可以在短是时间内增加基本经验值的100%，30分0秒,可使用。
            ...
            mCurrentTemp.AddHealthState(E_BattleSkillStats.UseGuangZhiZhuFu310061, b_Item.ConfigData.Value, b_Item.ConfigData.Value2, 0, mCreateFunc, mBattleComponent);
            mCurrentTemp.UpdateHealthState();

            mCurrentTemp.Data.DBBufflist[(int)E_BattleSkillStats.UseGuangZhiZhuFu310061] = mBattleComponent.CurrentTimeTick + b_Item.ConfigData.Value2;
