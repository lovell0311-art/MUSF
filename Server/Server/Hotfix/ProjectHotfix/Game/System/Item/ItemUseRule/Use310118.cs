...
namespace ETHotfix
{
    ...
    /// 生命提升
    /// </summary>
    [ItemUseRule(typeof(Use310118))]
    public class Use310118 : C_ItemUseRule<Player, Item, IResponse>
    {
        public override async Task Run(Player b_Player, Item b_Item, IResponse b_Response)
        {
            ...
            Func<CombatSource.BattleSyncTimerTask> mCreateFunc = () =>
            {
                ...
                mBattleSyncTimer.DisposeAction = (b_CombatSource, b_BattleComponent, b_TimerTask) =>
                {
                    ...
                    if (b_CombatSource.HealthStatsDic.TryGetValue(E_BattleSkillStats.Use310118, out var hp_Curse))
                    {
            ...
            if (mCurrentTemp.HealthStatsDic.ContainsKey(E_BattleSkillStats.Use310118))
            {
                mCurrentTemp.AddHealthState(E_BattleSkillStats.Use310118, b_Item.ConfigData.Value, b_Item.ConfigData.Value2, 0, mCreateFunc, mBattleComponent, E_SkillStatsResetType.RESET_NUMBER);
                mCurrentTemp.UpdateHealthState();
            }
            else
            {
                mCurrentTemp.AddHealthState(E_BattleSkillStats.Use310118, b_Item.ConfigData.Value, b_Item.ConfigData.Value2, 0, mCreateFunc, mBattleComponent);
                //mCurrentTemp.GamePropertyDic[E_GameProperty.PROP_HP_MAX] += b_Item.ConfigData.Value;
                mCurrentTemp.UpdateHealthState();
                ...
                mPropertyData.Value = mCurrentTemp.GetNumerialFunc(E_GameProperty.PROP_HP_MAX);
                mChangeValueMessage.Info.Add(mPropertyData);
                mPropertyData = new G2C_BattleKVData();

                mCurrentTemp.Player.Send(mChangeValueMessage);
            }
            mCurrentTemp.Data.DBBufflist[(int)E_BattleSkillStats.Use310118] = mBattleComponent.CurrentTimeTick + b_Item.ConfigData.Value2;
            ...
            Func<CombatSource.BattleSyncTimerTask> mCreateFunc = () =>
            {
                ...
                {
                    if (b_CombatSource.IsDisposeable) return;
                    b_CombatSource.HealthStatsDic.TryGetValue(E_BattleSkillStats.Use310118, out var varInfo);
                    if (varInfo == null) return;
                    var mDefense = b_CombatSource.GetNumerialFunc(E_GameProperty.PROP_HP_MAX);
                    var ValueI = mDefense - varInfo.CacheDatas[0].StrengthValue;

                    (b_CombatSource as GamePlayer).GamePropertyDic[E_GameProperty.PROP_HP_MAX] = ValueI;
            ...
            mCurrentTemp.Data.DBBufflist.TryGetValue((int)E_BattleSkillStats.Use310118, out var Value);
            if (mBattleComponent.CurrentTimeTick > Value)
                Value = mBattleComponent.CurrentTimeTick;

            mCurrentTemp.Data.DBBufflist[(int)E_BattleSkillStats.Use310118] = Value + b_Item.ConfigData.Value;
            mCurrentTemp.Data.Serialize();
