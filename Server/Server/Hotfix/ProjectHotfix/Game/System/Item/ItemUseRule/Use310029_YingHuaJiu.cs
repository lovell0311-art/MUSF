...
namespace ETHotfix
{
    ...
    /// 樱花酒
    /// </summary>
    [ItemUseRule(typeof(Use310029))]
    public class Use310029 : C_ItemUseRule<Player, Item, IResponse>
    {
        public override async Task Run(Player b_Player, Item b_Item, IResponse b_Response)
        {
            ...
            Func<CombatSource.BattleSyncTimerTask> mCreateFunc = () =>
            {
                ...
                mBattleSyncTimer.DisposeAction = (b_CombatSource, b_BattleComponent, b_TimerTask) =>
                {
                    ...
                    if (b_CombatSource.HealthStatsDic.TryGetValue(E_BattleSkillStats.UseYingHuaJiu310029, out var hp_Curse))
                    {
            ...
            mCurrentTemp.AddHealthState(E_BattleSkillStats.UseYingHuaJiu310029, b_Item.ConfigData.Value, b_Item.ConfigData.Value2, 0, mCreateFunc, mBattleComponent);
            mCurrentTemp.UpdateHealthState();

            mCurrentTemp.Data.DBBufflist[(int)E_BattleSkillStats.UseYingHuaJiu310029] = mBattleComponent.CurrentTimeTick + b_Item.ConfigData.Value2;
            ...
            mCurrentTemp.UnitData.Mp += 700;
            var mCurrentTempHpmax = mCurrentTemp.GetNumerialFunc(E_GameProperty.PROP_MP_MAX);
            if (mCurrentTemp.UnitData.Mp > mCurrentTempHpmax)
            {
                mCurrentTemp.UnitData.Mp = mCurrentTempHpmax;
            }
