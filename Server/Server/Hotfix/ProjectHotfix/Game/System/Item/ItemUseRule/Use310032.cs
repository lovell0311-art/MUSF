...
namespace ETHotfix
{
    ...
    /// 卓越一击卷轴
    /// </summary>
    [ItemUseRule(typeof(Use310032))]
    public class Use310032 : C_ItemUseRule<Player, Item, IResponse>
    {
        public override async Task Run(Player b_Player, Item b_Item, IResponse b_Response)
        {
            ...
            mCurrentTemp.AddHealthState(E_BattleSkillStats.Use310032, b_Item.ConfigData.Value, b_Item.ConfigData.Value2, 0, mCreateFunc, mBattleComponent);
            mCurrentTemp.UpdateHealthState();

            mCurrentTemp.Data.DBBufflist[(int)E_BattleSkillStats.Use310032] = mBattleComponent.CurrentTimeTick + b_Item.ConfigData.Value2;
