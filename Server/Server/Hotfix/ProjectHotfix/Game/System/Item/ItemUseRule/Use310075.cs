...
namespace ETHotfix
{
    ...
    /// 龙魂旗帜之魂
    /// </summary>
    [ItemUseRule(typeof(Use310075))]
    public class Use310075 : C_ItemUseRule<Player, Item, IResponse>
    {
        ...
        public override async Task Run(Player b_Player, Item b_Item, IResponse b_Response)
        {
            ...
            switch (r)
            { 
                case 0: DropId = 11008;break;
                case 1: DropId = 13008;break;
                case 2: DropId = 14008;break;
            }
            (int, int) Id = (DropId, 30);
            Enemy monster = CreateMonster(map, enemyJsonDic[MobId], PosX, PosY, b_Player.GameUserId, 11, Id,b_Item.ItemUID);
            G2C_CangBaoTuPosUpdate_notice notice2 = new G2C_CangBaoTuPosUpdate_notice()
            {
                ...
                PosY = PosY,
                NpcConfigID = MobId,
                TreasureType = ETreasureType.LongWangBaoZang,
                Id = b_Item.ItemUID
            };
            if (mGamePlayer.dBCharacterDroplimit.MGLsit == null)
                mGamePlayer.dBCharacterDroplimit.MGLsit = new Dictionary<(int, int), int>();

            if (mGamePlayer.dBCharacterDroplimit.MGLsit.TryGetValue((11008, 30), out var Value))
            {
                mGamePlayer.dBCharacterDroplimit.MGLsit.Remove((11008, 30));
                mGamePlayer.dBCharacterDroplimit.MGLsit.Add(Id, ++Value);
            }
            else if (mGamePlayer.dBCharacterDroplimit.MGLsit.ContainsKey((13008, 30)))
            {
                mGamePlayer.dBCharacterDroplimit.MGLsit.Remove((13008, 30));
                mGamePlayer.dBCharacterDroplimit.MGLsit.Add(Id, ++Value);
            }
            else if(mGamePlayer.dBCharacterDroplimit.MGLsit.ContainsKey((14008, 30)))
            {
                mGamePlayer.dBCharacterDroplimit.MGLsit.Remove((14008, 30));
                mGamePlayer.dBCharacterDroplimit.MGLsit.Add(Id, ++Value);
            }
            else
        ...
        private Enemy CreateMonster(MapComponent mMapComponent, Enemy_InfoConfig enemyConfig, int posX, int posY, long GamePlayerId, int DropId,(int,int) MgDrop,long itemUid)
        {
