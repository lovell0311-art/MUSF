...
namespace ETHotfix
{
    ...
    /// 一般合成-果实合成
    /// </summary>
    [SynthesisRule(typeof(GuoShiSynthesis))]
    public class GuoShiSynthesis : C_SynthesisRule<SynthesisComponent, List<Item>, Synthesis_InfoConfig, G2C_ItemsSynthesis,int>
    {
        ...
        public override async Task Run(SynthesisComponent synthesis, List<Item> itemList, Synthesis_InfoConfig config, G2C_ItemsSynthesis b_Response, int mClientFinalR)
        {
            ...
            {
                310064,310065,310066,310067,310068
            };
            //检查物品
            ...
            {
                Log.Warning($"方法: GuoShiSynthesis 前后概率不一致C:{mClientFinalR}S:{config.BaseSuccessRate}SMax:{config.MaxSuccessRate}");
            }
            //生成合成物品
