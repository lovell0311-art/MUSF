...
namespace ETHotfix
{
    ...
    /// 翅膀披风-混沌之翼合成
    /// </summary>
    [SynthesisRule(typeof(HunDunWingSynthesis))]
    public class HunDunWingSynthesis : C_SynthesisRule<SynthesisComponent, List<Item>, Synthesis_InfoConfig, G2C_ItemsSynthesis,int>
    {
        /// <summary>
        /// 出现幸运属性概率
        /// </summary>
        ...
        public override async Task Run(SynthesisComponent synthesis, List<Item> itemList, Synthesis_InfoConfig config, G2C_ItemsSynthesis b_Response, int mClientFinalR)
        {
            ...
            for (int i = 0; i < itemList.Count; i++)
            { 
                int value = itemList[i].GetProp(EItemValue.Quantity);
            ...
            {
                Log.Warning($"方法: HunDunWingSynthesis 前后概率不一致C:{mClientFinalR}S:{curSuccessRate}SMax:{config.MaxSuccessRate}");
            }
            //限制最大成功率
