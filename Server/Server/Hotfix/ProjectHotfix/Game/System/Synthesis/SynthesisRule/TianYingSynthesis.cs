...
namespace ETHotfix
{
    ...
    /// 坐骑护卫-天鹰
    /// </summary>
    [SynthesisRule(typeof(TianYingSynthesis))]
    public class TianYingSynthesis : C_SynthesisRule<SynthesisComponent, List<Item>, Synthesis_InfoConfig, G2C_ItemsSynthesis,int>
    {
        public override async Task Run(SynthesisComponent synthesis, List<Item> itemList, Synthesis_InfoConfig config, G2C_ItemsSynthesis b_Response, int mClientFinalR)
        {
            ...
            {
                Log.Warning($"方法: TianYingSynthesis 前后概率不一致C:{mClientFinalR}S:{curSuccessRate}SMax:{config.MaxSuccessRate}");
            }
            //生成合成物品
            Item resultItem = ItemFactory.Create(260015, synthesis.mPlayer.GameAreaId);
            //判断是否合成成功
