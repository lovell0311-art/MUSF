...
namespace ETHotfix
{
    ...
    ///坐骑护卫-圣迹龙魂(赤耀)
    /// </summary>
    [SynthesisRule(typeof(MarkDragonSoulSynthesis))]
    public class MarkDragonSoulSynthesis : C_SynthesisRule<SynthesisComponent, List<Item>, Synthesis_InfoConfig, G2C_ItemsSynthesis,int>
    {
        public override async Task Run(SynthesisComponent synthesis, List<Item> itemList, Synthesis_InfoConfig config, G2C_ItemsSynthesis b_Response, int mClientFinalR)
        {
            ...
            void DelItem(Item item, bool Del = true)
            {
                ...
                {
                    if (!synthesis.UseItem(Item.ItemUID, "合成消耗", Item.GetProp(EItemValue.Quantity)))
                    {
            ...
            {
                Log.Warning($"方法: MarkDragonSoulSynthesis 前后概率不一致C:{mClientFinalR}S:{curSuccessRate}SMax:{config.MaxSuccessRate}");
            }
            //限制最大成功率
