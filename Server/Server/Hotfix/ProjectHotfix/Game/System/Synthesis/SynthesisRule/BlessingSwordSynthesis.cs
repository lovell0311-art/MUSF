...
namespace ETHotfix
{
    ...
    /// 其他物品-祝福大天使之剑
    /// </summary>
    [SynthesisRule(typeof(BlessingSwordSynthesis))]
    public class BlessingSwordSynthesis : C_SynthesisRule<SynthesisComponent, List<Item>, Synthesis_InfoConfig, G2C_ItemsSynthesis,int>
    {
        public override async Task Run(SynthesisComponent synthesis, List<Item> itemList, Synthesis_InfoConfig config, G2C_ItemsSynthesis b_Response, int mClientFinalR)
        {
            ...
            int curSynItemConfigID = 10021;
            int NeedItemID = 10018; //所需的装备物品ID
            ...
            {
                Log.Warning($"方法: BlessingSwordSynthesis 前后概率不一致C:{mClientFinalR}S:{curSuccessRate}SMax:{config.MaxSuccessRate}");
            }
            //限制最大成功率
