...
namespace ETHotfix
{
    ...
    /// 其他装备-祝福大天使之杖合成
    /// </summary>
    [SynthesisRule(typeof(BlessingStaffSynthesis))]
    public class BlessingStaffSynthesis : C_SynthesisRule<SynthesisComponent, List<Item>, Synthesis_InfoConfig, G2C_ItemsSynthesis,int>
    {
        public override async Task Run(SynthesisComponent synthesis, List<Item> itemList, Synthesis_InfoConfig config, G2C_ItemsSynthesis b_Response, int mClientFinalR)
        {
            ...
            int curSynItemConfigID = 80014;
            int NeedItemID = 80010; //所需的装备物品ID
            ...
            {
                Log.Warning($"方法: BlessingStaffSynthesis 前后概率不一致C:{mClientFinalR}S:{curSuccessRate}SMax:{config.MaxSuccessRate}");
            }
            //限制最大成功率
