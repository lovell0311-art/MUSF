...
namespace ETHotfix
{
    ...
    /// 其他装备-祝福大天使绝对魔杖合成
    /// </summary>
    [SynthesisRule(typeof(BlessingWandSynthesis))]
    public class BlessingWandSynthesis : C_SynthesisRule<SynthesisComponent, List<Item>, Synthesis_InfoConfig, G2C_ItemsSynthesis,int>
    {
        public override async Task Run(SynthesisComponent synthesis, List<Item> itemList, Synthesis_InfoConfig config, G2C_ItemsSynthesis b_Response, int mClientFinalR)
        {
            ...
            int curSynItemConfigID = 80033;
            int NeedItemID = 80032; //所需的装备物品ID
            ...
            {
                Log.Warning($"方法: BlessingWandSynthesis 前后概率不一致C:{mClientFinalR}S:{curSuccessRate}SMax:{config.MaxSuccessRate}");
            }
            //限制最大成功率
