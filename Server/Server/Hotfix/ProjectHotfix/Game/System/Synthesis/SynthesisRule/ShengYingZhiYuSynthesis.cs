...
namespace ETHotfix
{
    ...
    /// 翅膀披风-神鹰之羽合成
    /// </summary>
    [SynthesisRule(typeof(ShengYingZhiYuSynthesis))]
    public class ShengYingZhiYuSynthesis : C_SynthesisRule<SynthesisComponent, List<Item>, Synthesis_InfoConfig, G2C_ItemsSynthesis,int>
    {
        ...
        public override async Task Run(SynthesisComponent synthesis, List<Item> itemList, Synthesis_InfoConfig config, G2C_ItemsSynthesis b_Response, int mClientFinalR)
        {
            ...
            for (int i = 0; i < itemList.Count; i++)
            {
                ...
                if (NeedItem.Contains(itemList[i].ConfigID) && optLevel >= 1 && level >= 9)
                {
            ...
            {
                b_Response.Error = Root.MainFactory.GetCustomComponent<LanguageComponent>().GetMessageId(1716);
                b_Response.Message = "二代翅膀/披风不存在或要求等级不足";
                return;
            }
            //增加成功率加成判定
            int ruleMaxCount = 10;
            ...
            {
                Log.Warning($"方法: ShengYingZhiYuSynthesis 前后概率不一致C:{mClientFinalR}S:{curSuccessRate}SMax:{config.MaxSuccessRate}");
            }
            //限制最大成功率
