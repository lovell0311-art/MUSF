...
namespace ETHotfix
{
    ...
    /// 翅膀披风-加鲁达之羽合成
    /// </summary>
    [SynthesisRule(typeof(JiaLuDaZhiYuSynthesis))]
    public class JiaLuDaZhiYuSynthesis : C_SynthesisRule<SynthesisComponent, List<Item>, Synthesis_InfoConfig, G2C_ItemsSynthesis,int>
    {
        ...
        {
            List<int> NeedItem = new List<int>() { 220027,
                220029,
                220030,
                220031,
                220032,
            ...
            for (int i = 0; i < itemList.Count; i++)
            {
                ...
                if (NeedItem.Contains(itemList[i].ConfigID) && optLevel >= 1 && level >= 13)
                {
            ...
            {
                b_Response.Error = Root.MainFactory.GetCustomComponent<LanguageComponent>().GetMessageId(1716);
                b_Response.Message = "三代翅膀/披风不存在或要求等级不足";
                return;
            }
            //增加成功率加成判定
            int ruleMaxCount = 10;
            ...
            {
                Log.Warning($"方法: JiaLuDaZhiYuSynthesis 前后概率不一致C:{mClientFinalR}S:{curSuccessRate}SMax:{config.MaxSuccessRate}");
            }
            //限制最大成功率
