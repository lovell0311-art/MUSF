...
namespace ETHotfix
{
    ...
    /// 坐骑护卫-炎狼兽之角+破坏合成
    /// </summary>
    [SynthesisRule(typeof(YanLangShouPoHuaiSynthesis))]
    public class YanLangShouPoHuaiSynthesis : C_SynthesisRule<SynthesisComponent, List<Item>, Synthesis_InfoConfig, G2C_ItemsSynthesis,int>
    {
        public override async Task Run(SynthesisComponent synthesis, List<Item> itemList, Synthesis_InfoConfig config, G2C_ItemsSynthesis b_Response, int mClientFinalR)
        {
            ...
            {
                Log.Warning($"方法: YanLangShouPoHuaiSynthesis 前后概率不一致C:{mClientFinalR}S:{curSuccessRate}SMax:{config.MaxSuccessRate}");
            }
            //限制最大成功率
