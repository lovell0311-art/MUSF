...
namespace ETHotfix
{
    ...
    /// 其他装备-玛雅武器合成
    /// </summary>
    [SynthesisRule(typeof(MayaWeaponSynthesis))]
    public class MayaWeaponSynthesis : C_SynthesisRule<SynthesisComponent, List<Item>, Synthesis_InfoConfig, G2C_ItemsSynthesis,int>
    {
        ...
        public override async Task Run(SynthesisComponent synthesis, List<Item> itemList, Synthesis_InfoConfig config, G2C_ItemsSynthesis b_Response, int mClientFinalR)
        {
            ...
            for (int i = 0; i < itemList.Count; i++)
            {
                ...
                if (itemList[i].GetProp(EItemValue.OptLevel) >= 1 && itemList[i].GetProp(EItemValue.Level) >= 4)
                {
                    NeedItem1 = itemList[i];
                    curSuccessRate += optLevel * 3;
                    curSuccessRate += level * 2;
                    if (itemList[i].data.ExcellentEntry.Count >= 1)
                    {
            ...
            {
                Log.Warning($"方法: MayaWeaponSynthesis 前后概率不一致C:{mClientFinalR}S:{curSuccessRate}SMax:{config.MaxSuccessRate}");
            }
            //限制最大成功率
