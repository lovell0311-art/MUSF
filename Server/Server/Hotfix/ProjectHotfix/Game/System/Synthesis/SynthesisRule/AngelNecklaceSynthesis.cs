...
namespace ETHotfix
{
    ...
    /// 天使项链
    /// </summary>
    [SynthesisRule(typeof(AngelNecklaceSynthesis))]
    public class AngelNecklaceSynthesis : C_SynthesisRule<SynthesisComponent, List<Item>, Synthesis_InfoConfig, G2C_ItemsSynthesis,int>
    {
        public override async Task Run(SynthesisComponent synthesis, List<Item> itemList, Synthesis_InfoConfig config, G2C_ItemsSynthesis b_Response, int mClientFinalR)
        {
            ...
            if (StrengthenItemSystem.StrengthenResult(curSuccessRate))
            {
                ...
                CreateAttr.Quantity = 1;
                CreateAttr.CustomAttrMethod.Add("ItemRandAddExcAttr_3_6");
                //生成合成物品
                Item resultItem = ItemFactory.Create(curSynItemConfigID, synthesis.mPlayer.GameAreaId, CreateAttr);
