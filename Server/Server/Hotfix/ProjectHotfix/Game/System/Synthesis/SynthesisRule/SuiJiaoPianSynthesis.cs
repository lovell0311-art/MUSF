...
namespace ETHotfix
{
    ...
    /// 坐骑护卫-碎角片合成
    /// </summary>
    [SynthesisRule(typeof(SuiJiaoPianSynthesis))]
    public class SuiJiaoPianSynthesis : C_SynthesisRule<SynthesisComponent, List<Item>, Synthesis_InfoConfig, G2C_ItemsSynthesis,int>
    {
        public override async Task Run(SynthesisComponent synthesis, List<Item> itemList, Synthesis_InfoConfig config, G2C_ItemsSynthesis b_Response, int mClientFinalR)
        {
            ...
            {
                Log.Warning($"方法: SuiJiaoPianSynthesis 前后概率不一致C:{mClientFinalR}S:{config.BaseSuccessRate}SMax:{config.MaxSuccessRate}");
            }
            //=================判定逻辑结束，开始消耗逻辑(所有异常放到上面报，下面默认判断为合成逻辑成功)==================
            synthesis.BackAllItemToBackpack(config.Info);
            //生成合成物品
            Item resultItem = ItemFactory.Create(320009, synthesis.mPlayer.GameAreaId);
            //判断是否合成成功
