...
namespace ETHotfix
{
    ...
    /// 宝石兑换藏宝图
    /// </summary>
    [SynthesisRule(typeof(GemExchangeTreasureMap))]
    public class GemExchangeTreasureMap : C_SynthesisRule<SynthesisComponent, List<Item>, Synthesis_InfoConfig, G2C_ItemsSynthesis, int>
    {
        ...
        {
            Dictionary<EItemStrengthen, int> ItemTreasureMap = new Dictionary<EItemStrengthen, int>()
            { {EItemStrengthen.CREATE_GEMS,60 },
            {EItemStrengthen.MAYA_GEMS,60 },
            {EItemStrengthen.RECYCLED_GEMS,60 },
            {EItemStrengthen.BLESSING_GEMS,10 },
            {EItemStrengthen.SOUL_GEMS,10 },
            {EItemStrengthen.ANIMA_GEMS,3 },
            };
            int curSuccessRate = config.BaseSuccessRate;
            int ItemConfigId = 310073;
            EItemStrengthen Type = EItemStrengthen.Wing_ResetCard;
            int Cnt = 0;
            //检查金币
            ...
            foreach (Item item in itemList)
            {
                ...
                int ItemCnt = item.GetProp(EItemValue.Quantity);
                switch (item.data.ConfigID)
                {
                    case (int)EItemStrengthen.CREATE_GEMS:
                        Cnt += ItemCnt;
                        Type = EItemStrengthen.CREATE_GEMS;
                        break;
                    case (int)EItemStrengthen.MAYA_GEMS:
                        Cnt += ItemCnt;
                        Type = EItemStrengthen.MAYA_GEMS;
                        break;
                    case (int)EItemStrengthen.RECYCLED_GEMS:
                        Cnt += ItemCnt;
                        Type = EItemStrengthen.RECYCLED_GEMS;
                        break;
                    case (int)EItemStrengthen.BLESSING_GEMS:
                        Cnt += ItemCnt;
                        Type = EItemStrengthen.BLESSING_GEMS;
                        break;
                    case (int)EItemStrengthen.SOUL_GEMS:
                        Cnt += ItemCnt;
                        Type = EItemStrengthen.SOUL_GEMS;
                        break;
                    case (int)EItemStrengthen.ANIMA_GEMS:
                        Cnt += ItemCnt;
                        Type = EItemStrengthen.ANIMA_GEMS;
                        break;
                }
            }
            if (ItemTreasureMap.TryGetValue(Type,out int Value) == false)
            {
                b_Response.Error = Root.MainFactory.GetCustomComponent<LanguageComponent>().GetMessageId(1716);
                b_Response.Message = "物品种类或数量不正确";
                return;
            ...
            Item resultItem = ItemFactory.Create(ItemConfigId, synthesis.mPlayer.GameAreaId);
