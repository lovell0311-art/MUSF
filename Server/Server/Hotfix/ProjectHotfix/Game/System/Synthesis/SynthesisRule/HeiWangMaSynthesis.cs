...
namespace ETHotfix
{
    ...
    /// 坐骑护卫-黑王马
    /// </summary>
    [SynthesisRule(typeof(HeiWangMaSynthesis))]
    public class HeiWangMaSynthesis : C_SynthesisRule<SynthesisComponent, List<Item>, Synthesis_InfoConfig, G2C_ItemsSynthesis,int>
    {
        public override async Task Run(SynthesisComponent synthesis, List<Item> itemList, Synthesis_InfoConfig config, G2C_ItemsSynthesis b_Response, int mClientFinalR)
        {
            ...
            {
                Log.Warning($"方法: HeiWangMaSynthesis 前后概率不一致C:{mClientFinalR}S:{curSuccessRate}SMax:{config.MaxSuccessRate}");
            }
            synthesis.BackAllItemToBackpack(config.Info);
            //生成合成物品
            Item resultItem = ItemFactory.Create(260012, synthesis.mPlayer.GameAreaId);
            //判断是否合成成功
