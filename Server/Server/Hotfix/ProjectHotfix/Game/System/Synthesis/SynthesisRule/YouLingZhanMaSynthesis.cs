...
namespace ETHotfix
{
    ...
    /// 坐骑护卫-幽灵战马
    /// </summary>
    [SynthesisRule(typeof(YouLingZhanMaSynthesis))]
    public class YouLingZhanMaSynthesis : C_SynthesisRule<SynthesisComponent, List<Item>, Synthesis_InfoConfig, G2C_ItemsSynthesis,int>
    {
        public override async Task Run(SynthesisComponent synthesis, List<Item> itemList, Synthesis_InfoConfig config, G2C_ItemsSynthesis b_Response, int mClientFinalR)
        {
            ...
            {
                Log.Warning($"方法: YouLingZhanMaSynthesis 前后概率不一致C:{mClientFinalR}S:{curSuccessRate}SMax:{config.MaxSuccessRate}");
            }
            //判断是否合成成功
            if (StrengthenItemSystem.StrengthenResult(curSuccessRate))
            {
                //生成合成物品
                Item resultItem = ItemFactory.Create(260007, synthesis.mPlayer.GameAreaId);
