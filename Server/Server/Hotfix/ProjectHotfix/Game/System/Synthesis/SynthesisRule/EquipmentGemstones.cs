...
namespace ETHotfix
{
    ...
    /// 龙王旗帜合成
    /// </summary>
    [SynthesisRule(typeof(EquipmentGemstones))]
    public class EquipmentGemstones : C_SynthesisRule<SynthesisComponent, List<Item>, Synthesis_InfoConfig, G2C_ItemsSynthesis,int>
    {
        public override async Task Run(SynthesisComponent synthesis, List<Item> itemList, Synthesis_InfoConfig config, G2C_ItemsSynthesis b_Response, int mClientFinalR)
        {
            ...
            foreach (Item item in itemList)
            {
                ...
                ExcellentEntryCount = item.data.ExcellentEntry.Count;
            }
            ...
            switch (ExcellentEntryCount)
            {
                case 0: return;
                case 1: 
                case 2: curSynItemConfigID = 280001; break;
                case 3: 
                case 4:
                    {
                        if(random.Next(1, 3) == 1)
                            curSynItemConfigID = 280003;
                        else
                            curSynItemConfigID = 280004;
                    }
                    break;
                case 5:
                case 6:
                    {
                        if (random.Next(1, 3) == 1)
                            curSynItemConfigID = 280005;
                        else
                            curSynItemConfigID = 280006;
                    }
                    break;
            }
            //生成合成物品
