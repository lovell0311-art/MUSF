...
namespace ETHotfix
{
    ...
    /// 地牢术
    /// </summary>
    [HeroSkillAttribute(BindId = (int)E_HeroSkillId.DiLaoShu326)]
    public partial class C_HeroSkill_DiLaoShu326 : C_HeroSkillSource
    {
        ...
        {
            if (b_Attacker.BattleMasteryDic.ContainsKey(E_BattleMasteryState.Skill_DiLaoShuStrengthen351))
            {
                return 3;
            }
            return CoolTime;
    ...
    /// 地牢术
    /// </summary>
    public partial class C_HeroSkill_DiLaoShu326
    {
        ...
        public override bool TryUse(CombatSource b_Attacker, CombatSource b_BeAttacker, C_FindTheWay2D b_Cell, BattleComponent b_BattleComponent, IActorResponse b_Response)
        {
            ...
            if (b_Attacker.BattleMasteryDic.ContainsKey(E_BattleMasteryState.Skill_DiLaoShu349) == false)
            {
        ...
        public override bool UseSkill(CombatSource b_Attacker, CombatSource b_BeAttacker, BattleComponent b_BattleComponent)
        {
            ...
            Action<long, long, long> mSyncAction = (b_CombatRoundId, b_AttackerId, b_BeAttackerId) =>
            {
                ...
                int GetHurtValue(E_GameProperty b_SpecialAttack)
                {
                    ...
                    if (b_Attacker.BattleMasteryDic.ContainsKey(E_BattleMasteryState.Skill_DiLaoShuStrengthen351))
                    {
                        int mMasteryValue = b_Attacker.BattleMasteryDic[E_BattleMasteryState.Skill_DiLaoShuStrengthen351];
                        ...
                        for (int j = 0, jlen = mFieldEnemylist.Length; j < jlen; j++)
                        {
                            ...
                            //if (Vector2.Distance(mSelfPos, new Vector2(mCurrentTemp.UnitData.X, mCurrentTemp.UnitData.Y)) < mRadius)
                            {
                                ...
                                if (IsHit)
                                {
                                    ...
                                    if (mCurrentTemp.Config.Monster_Type != 1)
                                    {// boss不受控
                                        var mRangeIndex = Help_RandomHelper.Range(0, 100);
                                        if (mRangeIndex > 50)
                                        {
                                            mCurrentTemp.AddHealthState(E_BattleSkillStats.DiLaoShu326, 0, mConfig.PersistentTime, 0, mCreateFunc, b_BattleComponent);
