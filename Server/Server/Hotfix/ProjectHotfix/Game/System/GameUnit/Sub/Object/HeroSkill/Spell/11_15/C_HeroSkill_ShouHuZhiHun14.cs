...
namespace ETHotfix
{
    ...
    /// 守护之魂
    /// </summary>
    [HeroSkillAttribute(BindId = (int)E_HeroSkillId.ShouHuZhiHun)]
    public partial class C_HeroSkill_ShouHuZhiHun14 : C_HeroSkillSource
    {
    ...
    /// 守护之魂
    /// </summary>
    public partial class C_HeroSkill_ShouHuZhiHun14
    {
        ...
        public override bool UseSkill(CombatSource b_Attacker, CombatSource b_BeAttacker, BattleComponent b_BattleComponent)
        {
            ...
            Action<long, long, long> mSyncAction = (b_CombatRoundId, b_AttackerId, b_BeAttackerId) =>
            {
                ...
                // 每次被攻击消耗魔法值 = 当前魔法值 * 2 %。
                // 守护之魂吸收百分比 = 10 + int(敏捷 / 50) + int(智力 / 200)。
                // 守护之魂持续时间为：60 + 智力 / 20

                var mAgility = b_Attacker.GetNumerialFunc(E_GameProperty.Property_Agility);
                var mWillpower = b_Attacker.GetNumerialFunc(E_GameProperty.Property_Willpower);

                int mStrengthValue = 10 + (int)(mAgility * 0.02f) + (int)(mWillpower * 0.0067f);
                mStrengthValue *= 100;
                if (mStrengthValue > 5000) mStrengthValue = 5000;

                int mPersistentTime = 60 + (int)(mWillpower * 0.05f);
                mPersistentTime *= 1000;

                if (b_Attacker.BattleMasteryDic.ContainsKey(E_BattleMasteryState.Skill_ShouHuZhiHunStrengthen53))
                {
                    int mMasteryValue = b_Attacker.BattleMasteryDic[E_BattleMasteryState.Skill_ShouHuZhiHunStrengthen53];
                    mStrengthValue += mMasteryValue;
                }
                if (b_Attacker.BattleMasteryDic.ContainsKey(E_BattleMasteryState.Skill_ShouHuZhiHunSkilled57))
                {
                    int mMasteryValue = b_Attacker.BattleMasteryDic[E_BattleMasteryState.Skill_ShouHuZhiHunSkilled57];
                    mPersistentTime += mMasteryValue;
                }
                ...
                Func<CombatSource.BattleSyncTimerTask> mCreateFunc = () =>
                {
                    ...
                    mBattleSyncTimer.DisposeAction = (b_CombatSource, b_BattleComponent, b_TimerTask) =>
                    {
                        ...
                        b_CombatSource.RemoveHealthState(E_BattleSkillStats.ShouHuZhiHun, b_BattleComponent);
                    ...
                    mBattleSyncTimer.SyncAction = (b_CombatSource, b_BattleComponent, b_TimerTask, b_TimeTick) =>
                    {
                        ...
                        if (b_CombatSource.HealthStatsDic.TryGetValue(E_BattleSkillStats.ShouHuZhiHun, out var hp_Curse) == false)
                        {
                ...
                {
                    // 对自身或者友军单体施加守护之魂效果。守护之魂：消耗BUFF所有者的当前魔法并吸收伤害，吸收量以及持续时间由BUFF提供者的属性决定.
                    b_BeAttacker.AddHealthState(E_BattleSkillStats.ShouHuZhiHun, mStrengthValue, mPersistentTime, 0, mCreateFunc, b_BattleComponent);
