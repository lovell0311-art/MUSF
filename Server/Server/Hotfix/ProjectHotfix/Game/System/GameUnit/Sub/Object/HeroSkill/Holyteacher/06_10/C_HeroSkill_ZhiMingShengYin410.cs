...
namespace ETHotfix
{
    ...
    /// 星云召唤
    /// </summary>
    [HeroSkillAttribute(BindId = (int)E_HeroSkillId.ZhiMingShengYin410)]
    public partial class C_HeroSkill_ZhiMingShengYin410 : C_HeroSkillSource
    {
    ...
    /// 星云召唤
    /// </summary>
    public partial class C_HeroSkill_ZhiMingShengYin410
    {
        ...
        public override bool UseSkill(CombatSource b_Attacker, CombatSource b_BeAttacker, BattleComponent b_BattleComponent)
        {
            ...
            Action<long, long, long> mSyncAction = (b_CombatRoundId, b_AttackerId, b_BeAttackerId) =>
            {
                ...
                int mCommand = b_Attacker.GetNumerialFunc(E_GameProperty.Property_Command);
                int mStrengthValue = mWillpower / 5 + mCommand / 6;

                if (b_Attacker.BattleMasteryDic.TryGetValue(E_BattleMasteryState.Skill_ZhiMingShengYinStrengthen429, out var mMasteryValue))
                {
                    mStrengthValue += mMasteryValue;
                }
                // 单位:秒
                int mPersistentTime = (int)(mWillpower / 42) + 30;

                if (b_Attacker.BattleMasteryDic.TryGetValue(E_BattleMasteryState.Skill_ZhiMingShengYinStrengthen2_432, out mMasteryValue))
                {
                ...
                Func<CombatSource.BattleSyncTimerTask> mCreateFunc = () =>
                {
                    ...
                    mBattleSyncTimer.DisposeAction = (b_CombatSource, b_BattleComponent, b_TimerTask) =>
                    {
                        ...
                        b_CombatSource.RemoveHealthState(E_BattleSkillStats.ZhiMingShengYin410, b_BattleComponent);
                    ...
                    mBattleSyncTimer.SyncAction = (b_CombatSource, b_BattleComponent, b_TimerTask, b_TimeTick) =>
                    {
                        ...
                        if (b_CombatSource.HealthStatsDic.TryGetValue(E_BattleSkillStats.ZhiMingShengYin410, out var hp_Curse) == false)
                        {
                ...
                {
                    b_BeAttacker.AddHealthState(E_BattleSkillStats.ZhiMingShengYin410, mStrengthValue, mPersistentTime, 0, mCreateFunc, b_BattleComponent);
                    b_BeAttacker.UpdateHealthState();
                    b_BeAttacker.ChangeNumerialType(E_GameProperty.LucklyAttackHurtValueIncrease);
                }
                action(b_Attacker);
