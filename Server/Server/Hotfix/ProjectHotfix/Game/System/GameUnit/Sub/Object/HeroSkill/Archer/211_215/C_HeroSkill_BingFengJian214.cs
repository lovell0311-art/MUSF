...
namespace ETHotfix
{
    ...
    /// 冰封箭
    /// </summary>
    [HeroSkillAttribute(BindId = (int)E_HeroSkillId.BingFengJian)]
    public partial class C_HeroSkill_BingFengJian214 : C_HeroSkillSource
    {
    ...
    /// 冰封箭
    /// </summary>
    public partial class C_HeroSkill_BingFengJian214
    {
        ...
        public override bool TryUse(CombatSource b_Attacker, CombatSource b_BeAttacker, C_FindTheWay2D b_Cell, BattleComponent b_BattleComponent, IActorResponse b_Response)
        {
            ...
            var mEquipmentComponent = (b_Attacker as GamePlayer).Player.GetCustomComponent<EquipmentComponent>();
            if (mEquipmentComponent.EquipPartItemDict.TryGetValue((int)EquipPosition.Weapon, out var mWeaponEquipment) == false)
            {
            ...
            switch (mWeaponEquipment.Type)
            {
                case EItemType.Bows:
                    {
                        if (mEquipmentComponent.EquipPartItemDict.TryGetValue((int)EquipPosition.Shield, out var mShieldEquipment) == false)
                        {
                            b_Response.Error = Root.MainFactory.GetCustomComponent<LanguageComponent>().GetMessageId(441);
                            //b_Response.Message = Root.MainFactory.GetCustomComponent<LanguageComponent>().GetMessage("目标已阵亡,不能攻击!");
                            return false;
                        ...
                        {
                            if (b_Attacker.HealthStatsDic.ContainsKey(E_BattleSkillStats.WuYingJian216) == false)
                            {
                                b_Response.Error = Root.MainFactory.GetCustomComponent<LanguageComponent>().GetMessageId(441);
                                //b_Response.Message = Root.MainFactory.GetCustomComponent<LanguageComponent>().GetMessage("目标已阵亡,不能攻击!");
                                return false;
        ...
        public override bool UseSkill(CombatSource b_Attacker, CombatSource b_BeAttacker, BattleComponent b_BattleComponent)
        {
            ...
            Action<long, long, long> mSyncAction = (b_CombatRoundId, b_AttackerId, b_BeAttackerId) =>
            {
                ...
                int GetHurtValue(E_GameProperty b_SpecialAttack)
                {
                    ...
                    if (b_Attacker.BattleMasteryDic.TryGetValue(E_BattleMasteryState.Skill_BingJianStrengthen_237, out var mMasteryValue))
                    {
                ...
                Func<CombatSource.BattleSyncTimerTask> mCreateFunc = () =>
                {
                    ...
                    mBattleSyncTimer.DisposeAction = (b_CombatSource, b_BattleComponent, b_TimerTask) =>
                    {
                        ...
                        b_CombatSource.RemoveHealthState(E_BattleSkillStats.BingFengJian214, b_BattleComponent);
                    ...
                    mBattleSyncTimer.SyncAction = (b_CombatSource, b_BattleComponent, b_TimerTask, b_TimeTick) =>
                    {
                        ...
                        if (b_CombatSource.HealthStatsDic.TryGetValue(E_BattleSkillStats.BingFengJian214, out var hp_Curse) == false)
                        {
                ...
                // 朝敌方单体射出冰封箭造成伤害并概率让目标无法动弹
                // 控制概率：if (r  == 0 )，必定控制；if (  (rand()%(r+1)) == 0 )，概率控制,r为目标冰抗性。
                // 减速
                var mIceResistance = mBeAttacker.GetNumerialFunc(E_GameProperty.IceResistance);
                if (mIceResistance == 0)
                {
                    mBeAttacker.AddHealthState(E_BattleSkillStats.BingFengJian214, 0, mConfig.PersistentTime, 0, mCreateFunc, b_BattleComponent);
