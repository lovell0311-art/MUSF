...
namespace ETHotfix
{
    ...
    /// 单体攻击
    /// </summary>
    [HeroSkillAttribute(BindId = (int)E_HeroSkillId.TianYing10022)]
    public partial class C_HeroSkill_TianYing10022 : C_HeroSkillSource
    {
    ...
    /// 单体攻击
    /// </summary>
    public partial class C_HeroSkill_TianYing10022
    {
        public bool TryUseByUseStandard(CombatSource b_Attacker, IActorResponse b_Response)
        {
            if (b_Attacker.Identity == E_Identity.Hero)
            {
                //b_Response.Error = Root.MainFactory.GetCustomComponent<LanguageComponent>().GetMessageId(433);
                return false;
            }
            return true;
        }
        ...
        public override bool TryUse(CombatSource b_Attacker, CombatSource b_BeAttacker, C_FindTheWay2D b_Cell, BattleComponent b_BattleComponent, IActorResponse b_Response)
        {
            if (!(Config is Skill_monsterConfig mConfig))
            {
                //Root.MainFactory.GetCustomComponent<LanguageComponent>().GetMessageId(425);
                return false;
            }

            if (Vector2.Distance(b_BattleComponent.Parent.GetFindTheWay2D(b_Attacker).Vector2Pos, b_Cell.Vector2Pos) > mConfig.Distance)
            {
                //Root.MainFactory.GetCustomComponent<LanguageComponent>().GetMessageId(411);
                return false;
            }
            ...
            switch (b_BeAttacker.Identity)
            {
                case E_Identity.Hero:
                    {
                        var mBeAttacker = b_BeAttacker as GamePlayer;
                        if (mBeAttacker.InstanceId == mPlayerInstance)
                        {
                            return false;
                        }
                        var mTryUseResult = b_Attacker.TryUseByPlayerKilling(mBeAttacker, b_Response);
                ...
                case E_Identity.Pet:
                    {
                        var mBeAttacker = b_BeAttacker as Pets;
                        if (mBeAttacker.GamePlayer.InstanceId == mPlayerInstance)
                        {
                            return false;
                        }
                ...
                case E_Identity.Summoned:
                    {
                        var mBeAttacker = b_BeAttacker as Summoned;
                        if (mBeAttacker.GamePlayer.InstanceId == mPlayerInstance)
                        {
                            return false;
                        }
        ...
        public override bool UseSkill(CombatSource b_Attacker, CombatSource b_BeAttacker, BattleComponent b_BattleComponent)
        {
            ...
            Action<long, long, long> mSyncAction = (b_CombatRoundId, b_AttackerId, b_BeAttackerId) =>
            {
                ...
                {
                    mVirtualHurtValue = b_Attacker.GetNumerialFunc(E_GameProperty.MaxAtteck) + b_Attacker.GetNumerialFunc(E_GameProperty.EmbedLuckyStrokeAttack) + b_Attacker.GetNumerialFunc(E_GameProperty.EmbedSkillAttack);
                }
                else
                {
