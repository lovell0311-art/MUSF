...
namespace ETHotfix
{
    ...
    /// 暴风雪
    /// </summary>
    [HeroSkillAttribute(BindId = (int)E_HeroSkillId.BaoFengXue)]
    public partial class C_HeroSkill_BaoFengXue19 : C_HeroSkillSource
    {
    ...
    /// 暴风雪
    /// </summary>
    public partial class C_HeroSkill_BaoFengXue19
    {
        ...
        public override bool UseSkill(CombatSource b_Attacker, CombatSource b_BeAttacker, BattleComponent b_BattleComponent)
        {
            ...
            Action<long, long, long> mSyncAction = (b_CombatRoundId, b_AttackerId, b_BeAttackerId) =>
            {
                ...
                int GetHurtValue(E_GameProperty b_SpecialAttack)
                {
                    ...
                    if (b_Attacker.BattleMasteryDic.ContainsKey(E_BattleMasteryState.Skill_BaoFengXueStrengthen40))
                    {
                        int mMasteryValue = b_Attacker.BattleMasteryDic[E_BattleMasteryState.Skill_BaoFengXueStrengthen40];
                        ...
                        for (int j = 0, jlen = mFieldEnemylist.Length; j < jlen; j++)
                        {
                            ...
                            //if (Vector2.Distance(mSelfPos, new Vector2(mCurrentTemp.UnitData.X, mCurrentTemp.UnitData.Y)) < mRadius)
                            {
                                ...
                                if (IsHit)
                                {
                                    ...
                                    // 减速
                                    var mIceResistance = mCurrentTemp.GetNumerialFunc(E_GameProperty.IceResistance);
                                    if (mIceResistance == 0)
                                    {
                                        mCurrentTemp.AddHealthState(E_BattleSkillStats.BingFeng, 0, mConfig.PersistentTime, 0, mCreateFunc, b_BattleComponent);
