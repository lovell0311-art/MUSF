...
namespace ETHotfix
{
    ...
    /// 整套防御力增加
    /// </summary>
    [BattleMasterAttribute(BindId = (int)E_BattleMaster_Id.MasterAttribute913)]
    public partial class C_BattleMaster_MasterAttribute913 : C_BattleMaster
    {
    ...
    /// 整套防御力增加
    /// </summary>
    public partial class C_BattleMaster_MasterAttribute913
    {
        ...
        public override bool UseSkill(GamePlayer b_Attacker, BattleComponent b_BattleComponent, bool b_AddPoint)
        {
            ...
            // 头盔、铠甲、护腿、护手、靴子等防具，如果装备同一种类时，防御力增加{0:F}。

            int mMasterLevel = Data.SkillId[this.Id];
            b_Attacker.BattleMasteryDic[E_BattleMasteryState.MasterAttribute913] = base.GetMasterValue(mConfig.OtherDataDic[mMasterLevel], mConfig.OtherDataUse);

            if (b_AddPoint)
            {
                b_Attacker.Player.GetCustomComponent<EquipmentComponent>()?.ApplyEquipProp();
            }
