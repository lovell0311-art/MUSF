...
namespace ETHotfix
{
    ...
    /// 安魂弥撒
    /// </summary>
    [HeroSkillAttribute(BindId = (int)E_HeroSkillId.AnHunMiSha505)]
    public partial class C_HeroSkill_AnHunMiSha505 : C_HeroSkillSource
    {
    ...
    /// 安魂弥撒
    /// </summary>
    public partial class C_HeroSkill_AnHunMiSha505
    {
        ...
        public override bool UseSkill(CombatSource b_Attacker, CombatSource b_BeAttacker, BattleComponent b_BattleComponent)
        {
            ...
            Action<long, long, long> mSyncAction = (b_CombatRoundId, b_AttackerId, b_BeAttackerId) =>
            {
                ...
                mAddScale += b_Attacker.GetNumerialFunc(E_GameProperty.EmbedSkillAttack); 
                ...
                int GetHurtValue(E_GameProperty b_SpecialAttack)
                {
                    ...
                    {
                        mVirtualHurtValue = mMaxAtteck + b_Attacker.GetNumerialFunc(E_GameProperty.EmbedLuckyStrokeAttack);
                    }
                    else if (b_SpecialAttack == E_GameProperty.ExcellentAttackRate)
                    {
                        mVirtualHurtValue = Help_RandomHelper.Range(mMinAtteck, mMaxAtteck)+ b_Attacker.GetNumerialFunc(E_GameProperty.EmbedGreatShotAttack);
                    }
                    else
                    {
                ...
                Func<CombatSource.BattleSyncTimerTask> mCreateFunc = () =>
                {
                    ...
                    mBattleSyncTimer.DisposeAction = (b_CombatSource, b_BattleComponent, b_TimerTask) =>
                    {
                        ...
                        b_CombatSource.RemoveHealthState(E_BattleSkillStats.AnHunMiSha505, b_BattleComponent);
                    ...
                    mBattleSyncTimer.SyncAction = (b_CombatSource, b_BattleComponent, b_TimerTask, b_TimeTick) =>
                    {
                        ...
                        if (b_CombatSource.HealthStatsDic.TryGetValue(E_BattleSkillStats.AnHunMiSha505, out var hp_Curse) == false)
                        {
                            return CombatSource.E_SyncTimerTaskResult.Dispose;
                        }

                        var mStateBuffers = hp_Curse.CacheDatas.Values.ToArray();
                        for (int i = 0, len = mStateBuffers.Length; i < len; i++)
                        {
                            var mStateBuffer = mStateBuffers[i];

                            if (mStateBuffer.IsDisposeable == false)
                            {
                                if (mStateBuffer.StrengthValue == 0) continue;

                                switch (b_CombatSource.Identity)
                                {
                                    case E_Identity.Enemy:
                                        {
                                            Enemy mEnemy = b_CombatSource as Enemy;

                                            mEnemy.InjureSkill(b_Attacker, E_BattleHurtType.MAGIC, E_GameProperty.NullAttack, Id, mStateBuffer.StrengthValue, b_BattleComponent);
                                        }
                                        break;
                                    case E_Identity.Summoned:
                                        {
                                            Summoned mSummoned = b_CombatSource as Summoned;

                                            mSummoned.InjureSkill(b_Attacker, E_BattleHurtType.MAGIC, E_GameProperty.NullAttack, Id, mStateBuffer.StrengthValue, b_BattleComponent);
                                        }
                                        break;
                                    case E_Identity.Pet:
                                        {
                                            Pets mPets = b_CombatSource as Pets;

                                            mPets.InjureSkill(b_Attacker, E_BattleHurtType.MAGIC, E_GameProperty.NullAttack, Id, mStateBuffer.StrengthValue, b_BattleComponent);
                                        }
                                        break;
                                    case E_Identity.Hero:
                                        {
                                            GamePlayer mGamePlayer = b_CombatSource as GamePlayer;

                                            mGamePlayer.InjureSkill(b_Attacker, E_BattleHurtType.MAGIC, E_GameProperty.NullAttack, Id, mStateBuffer.StrengthValue, b_BattleComponent);
                                        }
                                        break;
                                    default:
                                        break;
                        ...
                        for (int j = 0, jlen = mFieldEnemylist.Length; j < jlen; j++)
                        {
                            ...
                            //if (Vector2.Distance(mSelfPos, new Vector2(mCurrentTemp.UnitData.X, mCurrentTemp.UnitData.Y)) < mRadius)
                            {
                                ...
                                if (IsHit)
                                {
                                    ...
                                    int mBufferHurtValue = (int)(mHurtValue * 0.6f);
                                    mCurrentTemp.AddHealthState(E_BattleSkillStats.AnHunMiSha505, mBufferHurtValue, mConfig.PersistentTime, 0, mCreateFunc, b_BattleComponent);
