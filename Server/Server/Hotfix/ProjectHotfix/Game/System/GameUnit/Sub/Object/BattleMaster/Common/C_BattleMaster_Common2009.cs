...
namespace ETHotfix
{
    ...
    /// 剑雨
    /// </summary>
    [BattleMasterAttribute(BindId = (int)E_BattleMaster_Id.Common2009)]
    public partial class C_BattleMaster_Common2009 : C_BattleMaster
    {
    ...
    /// 剑雨
    /// </summary>
    public partial class C_BattleMaster_Common2009
    {
        #region try
        ...
        public override bool UseSkill(GamePlayer b_Attacker, BattleComponent b_BattleComponent, bool b_AddPoint)
        {
            ...
            b_Attacker.BattleMasteryDic[E_BattleMasteryState.Common2009] = 1;

            var mStudySkillId = (int)E_HeroSkillId.JianYu126;
            if (b_Attacker.Data.PlayerTypeId == (int)E_GameOccupation.Swordsman)
            {
                mStudySkillId = (int)E_HeroSkillId.JianYu126;
            ...
            else if (b_Attacker.Data.PlayerTypeId == (int)E_GameOccupation.Spellsword)
            {
                mStudySkillId = (int)E_HeroSkillId.JianYu335;
            ...
            else if (b_Attacker.Data.PlayerTypeId == (int)E_GameOccupation.Combat)
            {
                mStudySkillId = (int)E_HeroSkillId.JianYu618;

                var mReadConfigComponent = Root.MainFactory.GetCustomComponent<CustomFrameWork.Component.ReadConfigComponent>();
                var mJsonDic = mReadConfigComponent.GetJson<Skill_CombatConfigJson>().JsonDic;
                if (mJsonDic.TryGetValue(mStudySkillId, out var mSkillConfig) == false)
                {
                    return false;
                }
            }
            else if (b_Attacker.Data.PlayerTypeId == (int)E_GameOccupation.GrowLancer)
            {
                mStudySkillId = (int)E_HeroSkillId.JianYu714;

                var mReadConfigComponent = Root.MainFactory.GetCustomComponent<CustomFrameWork.Component.ReadConfigComponent>();
                var mJsonDic = mReadConfigComponent.GetJson<Skill_DreamKnightConfigJson>().JsonDic;
                if (mJsonDic.TryGetValue(mStudySkillId, out var mSkillConfig) == false)
                {
                    return false;
                }
