...
namespace ETHotfix
{
    ...
    /// 圣极光
    /// </summary>
    [HeroSkillAttribute(BindId = (int)E_HeroSkillId.ShengJiGuang411)]
    public partial class C_HeroSkill_ShengJiGuang411 : C_HeroSkillSource
    {
    ...
    /// 圣极光
    /// </summary>
    public partial class C_HeroSkill_ShengJiGuang411
    {
        ...
        public override bool UseSkill(CombatSource b_Attacker, CombatSource b_BeAttacker, BattleComponent b_BattleComponent)
        {
            ...
            Action<long, long, long> mSyncAction = (b_CombatRoundId, b_AttackerId, b_BeAttackerId) =>
            {
                ...
                int mMinAtteck = b_Attacker.GetNumerialFunc(E_GameProperty.MinAtteck);
                int mMaxAtteck = b_Attacker.GetNumerialFunc(E_GameProperty.MaxAtteck);
                ...
                int GetHurtValue(E_GameProperty b_SpecialAttack)
                {
                    ...
                    {
                        mVirtualHurtValue = mConfig.OtherDataDic[1] + mMaxAtteck+ b_Attacker.GetNumerialFunc(E_GameProperty.EmbedLuckyStrokeAttack) + b_Attacker.GetNumerialFunc(E_GameProperty.EmbedSkillAttack);
                    }
                    else
                    {
                        mVirtualHurtValue = mConfig.OtherDataDic[1] + Help_RandomHelper.Range(mMinAtteck, mMaxAtteck) + b_Attacker.GetNumerialFunc(E_GameProperty.EmbedSkillAttack);
                    ...
                    if (b_Attacker.BattleMasteryDic.ContainsKey(E_BattleMasteryState.Skill_ShengJiGuangStrengthen437))
                    {
                        int mMasteryValue = b_Attacker.BattleMasteryDic[E_BattleMasteryState.Skill_ShengJiGuangStrengthen437];
                ...
                // 敌方群体
                // 指定方向矩形，长度3，宽度2
                // 以自身为中心半径6范围内的包括自身以及队友，消耗其生命和魔法，对敌方目标及其直线3的矩形范围内所有敌人造成伤害
                {
                    int mAddHp = 0, mAddMp = 0;
                    void action(CombatSource b_BeAttacker)
                    {
                        var mChangeHpValue = (int)(b_BeAttacker.GetNumerialFunc(E_GameProperty.PROP_HP) * 0.8f);
                        var mChangeMpValue = (int)(b_BeAttacker.GetNumerialFunc(E_GameProperty.PROP_MP) * 0.95f);

                        mAddHp += (b_BeAttacker.UnitData.Hp - mChangeHpValue);
                        mAddMp += (b_BeAttacker.UnitData.Mp - mChangeMpValue);
                        b_BeAttacker.UnitData.Hp = mChangeHpValue;
                        b_BeAttacker.UnitData.Mp = mChangeMpValue;
                        ...
                        mData.Value = b_BeAttacker.GetNumerialFunc(E_GameProperty.PROP_HP);
                        mChangeValueMessage.Info.Add(mData);
                        mData = new G2C_BattleKVData();
                    ...
                    if (mAddHp > 0 || mAddMp > 0)
                    {
                        ...
                        if (mAddHp > 0)
                        {
                            ...
                            var mHpMax = b_Attacker.GetNumerialFunc(E_GameProperty.PROP_HP_MAX);
                            if (b_Attacker.UnitData.Hp > mHpMax) b_Attacker.UnitData.Hp = mHpMax;

                            G2C_BattleKVData mData = new G2C_BattleKVData();
                            mData.Key = (int)E_GameProperty.PROP_HP;
                            mData.Value = b_Attacker.GetNumerialFunc(E_GameProperty.PROP_HP);
                            mChangeValueMessage.Info.Add(mData);
                        }
                        if (mAddHp > 0)
                        {
                            b_Attacker.UnitData.Mp += mAddMp;
                            var mMpMax = b_Attacker.GetNumerialFunc(E_GameProperty.PROP_MP_MAX);
                            if (b_Attacker.UnitData.Mp > mMpMax) b_Attacker.UnitData.Mp = mMpMax;

                            G2C_BattleKVData mData = new G2C_BattleKVData();
                            mData.Key = (int)E_GameProperty.PROP_MP;
                            mData.Value = b_Attacker.GetNumerialFunc(E_GameProperty.PROP_MP);
                            mChangeValueMessage.Info.Add(mData);
                        }
