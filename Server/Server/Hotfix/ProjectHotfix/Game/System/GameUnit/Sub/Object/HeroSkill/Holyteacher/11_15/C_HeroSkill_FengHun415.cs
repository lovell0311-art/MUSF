...
namespace ETHotfix
{
    ...
    /// 风魂
    /// </summary>
    [HeroSkillAttribute(BindId = (int)E_HeroSkillId.FengHun415)]
    public partial class C_HeroSkill_FengHun415 : C_HeroSkillSource
    {
    ...
    /// 风魂
    /// </summary>
    public partial class C_HeroSkill_FengHun415
    {
        ...
        public override bool UseSkill(CombatSource b_Attacker, CombatSource b_BeAttacker, BattleComponent b_BattleComponent)
        {
            ...
            Action<long, long, long> mSyncAction = (b_CombatRoundId, b_AttackerId, b_BeAttackerId) =>
            {
                ...
                int GetHurtValue(E_GameProperty b_SpecialAttack)
                {
                    ...
                    {
                        mVirtualHurtValue = mConfig.OtherDataDic[1] + Help_RandomHelper.Range(mMinAtteck, mMaxAtteck) + b_Attacker.GetNumerialFunc(E_GameProperty.EmbedGreatShotAttack) + b_Attacker.GetNumerialFunc(E_GameProperty.EmbedSkillAttack);
                    }
                    else
                    {
                        ...
                        for (int j = 0, jlen = mFieldEnemylist.Length; j < jlen; j++)
                        {
                            ...
                            //if (Vector2.Distance(mSelfPos, new Vector2(mCurrentTemp.UnitData.X, mCurrentTemp.UnitData.Y)) < mRadius)
                            {
                                ...
                                if (IsHit)
                                {
                                    ...
                                    mCurrentTemp.InjureSkill(b_Attacker, E_BattleHurtType.MAGIC, mSpecialAttack, Id, GetHurtValue(mSpecialAttack), b_BattleComponent);
                                }
                                else
                                {
                                    G2C_AttackResult_notice mAttackResultNotice = new G2C_AttackResult_notice();
