...
namespace ETHotfix
{
    ...
    public class CombatSourceEnterOrSwitchMap_PetsSwitchMap : ITEventMethodOnRun<ETModel.EventType.CombatSourceEnterOrSwitchMap>
    {
        public void OnRun(ETModel.EventType.CombatSourceEnterOrSwitchMap args)
        {
            ...
            if (gamePlayer.Pets == null || gamePlayer.Pets.IsDisposeable == true) return;
            if (gamePlayer.Pets.IsDeath == true)
            {
                Log.PLog($"宠物死亡，强制传送 {gamePlayer.Pets.InstanceId}");
            }
            if (gamePlayer.Pets.CurrentMap == null) return; // 不在地图中

            // 清除单位的移动路径
            gamePlayer.Pets.Pathlist = null;
            // 将宠物传送到玩家身边
            gamePlayer.Pets.SwitchMap(
    ...
    {
        public static void Attack(this Pets b_Attacker, CombatSource b_BeAttacker, BattleComponent b_BattleComponent, bool b_CanBackInjure = true)
        {
            ...
            Action<long, long, long> mSyncAction = (b_CombatRoundId, b_AttackerId, b_BeAttackerId) =>
            {
                ...
                {
                    IsHit = b_Attacker.PetsIsHitPvP(b_BeAttacker, b_BattleComponent, true);
                }
                else
                {
                ...
                else
                {
                    ...
                    {
                        if (b_Attacker.GameHeroSexType == E_GameHeroSexType.BOY)
                            mVirtualInjureValue = b_Attacker.GetNumerialFunc(E_GameProperty.MaxAtteck);
                        else
                            mVirtualInjureValue = b_Attacker.GetNumerialFunc(E_GameProperty.MaxMagicAtteck);
                    }
                    else
                    {
                        if (b_Attacker.GameHeroSexType == E_GameHeroSexType.BOY)
                            ...
                            mVirtualInjureValue = Help_RandomHelper.Range(b_Attacker.GetNumerialFunc(E_GameProperty.MinMagicAtteck), b_Attacker.GetNumerialFunc(E_GameProperty.MaxMagicAtteck));
                    }
        ...
        public static int Injure(this Pets b_BeAttacker, CombatSource b_Attacker,
            ...
            bool b_CanBackInjure = true)
        {
            ...
            if (b_BeAttacker.dBPetsData.PetsHP <= mRealInjureValue)
            {
                b_BeAttacker.dBPetsData.PetsHP = 0;
                ...
                b_BeAttacker.dBPetsData.DeathTime = b_BattleComponent.CurrentTimeTick;

                var mFindTheWay = mMapComponent.GetFindTheWay2D(b_BeAttacker);

                var Playerinfo = b_BeAttacker.GamePlayer.Player;
                if (Playerinfo != null)
                {
                    var equipComponent = Playerinfo.GetCustomComponent<EquipmentComponent>();
            ...
            {
                b_BeAttacker.dBPetsData.PetsHP -= mRealInjureValue;
            ...
            mAttackResultNotice.HpValue = b_BeAttacker.dBPetsData.PetsHP;
            mAttackResultNotice.HpMaxValue = b_BeAttacker.GetNumerialFunc(E_GameProperty.PROP_HP_MAX);
            //mAttackResultNotice.MpValue = b_BeAttacker.dBPetsData.PetsMP;
            //mAttackResultNotice.MpMaxValue = b_BeAttacker.GetNumerialFunc(E_GameProperty.PROP_MP_MAX);
            mMapComponent.SendNotice(b_BeAttacker, mAttackResultNotice);

            if (b_BeAttacker.GamePlayer != null && b_BeAttacker.GamePlayer.Player != null)
            {
                DBProxyManagerComponent mDBProxyManager = Root.MainFactory.GetCustomComponent<DBProxyManagerComponent>();
                var dBProxy = mDBProxyManager.GetZoneDB(DBType.Core, (int)b_BeAttacker.GamePlayer.Player.GameAreaId);
                var mWriteDataComponent = Root.MainFactory.GetCustomComponent<DBMongodbProxySaveManageComponent>().Get((int)b_BeAttacker.GamePlayer.Player.GameAreaId);
                mWriteDataComponent.Save(b_BeAttacker.dBPetsData, dBProxy).Coroutine();
            }
            return mRealInjureValue;
        }

        public static int InjureSkill(this Pets b_BeAttacker, CombatSource b_Attacker,
            E_BattleHurtType b_BattleHurtType,
            E_GameProperty b_SpecialAttack,
            int b_HurtTypeId,
            int b_InjureValue,
        ...
        public static E_GameProperty AttackSpecial(this Pets b_Attacker)
        {
            ...
            else if (mRandomValue < b_Attacker.GetNumerialFunc(E_GameProperty.InjuryValueRate_2))
            {
                return E_GameProperty.InjuryValueRate_2;
            }
            else if (mRandomValue < b_Attacker.GetNumerialFunc(E_GameProperty.ExcellentAttackRate))
            {
                return E_GameProperty.ExcellentAttackRate;
            }
            else if (mRandomValue < b_Attacker.GetNumerialFunc(E_GameProperty.LucklyAttackRate))
            {
