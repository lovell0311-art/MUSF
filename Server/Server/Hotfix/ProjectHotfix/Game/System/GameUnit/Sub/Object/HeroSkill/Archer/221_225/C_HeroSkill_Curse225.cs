...
namespace ETHotfix
{
    ...
    /// 毒箭 Curse
    /// </summary>
    [HeroSkillAttribute(BindId = (int)E_HeroSkillId.Curse225)]
    public partial class C_HeroSkill_Curse225 : C_HeroSkillSource
    {
    ...
    /// 毒箭
    /// </summary>
    public partial class C_HeroSkill_Curse225
    {
        ...
        public override bool UseSkill(CombatSource b_Attacker, CombatSource b_BeAttacker, BattleComponent b_BattleComponent)
        {
            ...
            Action<long, long, long> mSyncAction = (b_CombatRoundId, b_AttackerId, b_BeAttackerId) =>
            {
                ...
                var mAgility = b_Attacker.GetNumerialFunc(E_GameProperty.Property_Agility) / mConfig.OtherDataDic[10];
                ...
                int GetHurtValue(E_GameProperty b_SpecialAttack)
                {
                    ...
                    {
                        mVirtualHurtValue = mConfig.OtherDataDic[1] + mMaxMagicAtteck + b_Attacker.GetNumerialFunc(E_GameProperty.EmbedLuckyStrokeAttack) + b_Attacker.GetNumerialFunc(E_GameProperty.EmbedSkillAttack);
                    }
                    else if (b_SpecialAttack == E_GameProperty.ExcellentAttackRate)
                    {
                        mVirtualHurtValue = mConfig.OtherDataDic[1] + Help_RandomHelper.Range(mMinMagicAtteck, mMaxMagicAtteck) + b_Attacker.GetNumerialFunc(E_GameProperty.EmbedGreatShotAttack) + b_Attacker.GetNumerialFunc(E_GameProperty.EmbedSkillAttack);
                    }
                     else
                    {
                    ...
                    int mHurtValue = mVirtualHurtValue + mAgility;
                    if (mAddScale != 100)
                        mHurtValue = (int)(mHurtValue * mAddScale * 0.01f);

                    if (b_Attacker.BattleMasteryDic.TryGetValue(E_BattleMasteryState.Skill_DuJianStrengthen_251, out var mMasteryValue))
                    {
                        mHurtValue += mMasteryValue;
                    }
                ...
                Func<CombatSource.BattleSyncTimerTask> mCreateFunc = () =>
                {
                    ...
                    mBattleSyncTimer.SyncWaitTime = 1 * 1000;
                    ...
                    mBattleSyncTimer.DisposeAction = (b_CombatSource, b_BattleComponent, b_TimerTask) =>
                    {
                        ...
                        b_CombatSource.RemoveHealthState(E_BattleSkillStats.Curse225, b_BattleComponent);
                    ...
                    mBattleSyncTimer.SyncAction = (b_CombatSource, b_BattleComponent, b_TimerTask, b_TimeTick) =>
                    {
                        ...
                        if (b_CombatSource.HealthStatsDic.TryGetValue(E_BattleSkillStats.Curse225, out var hp_Curse) == false)
                        {
                            return CombatSource.E_SyncTimerTaskResult.Dispose;
                        }

                        if (hp_Curse.CacheDatas.TryGetValue(0, out var mStatsDicTemp) == false)
                        {
                            return CombatSource.E_SyncTimerTaskResult.Dispose;
                        }

                        var mCurseHurtValue = mStatsDicTemp.StrengthValue;

                        switch (mBeAttacker.Identity)
                        {
                            case E_Identity.Enemy:
                                (mBeAttacker as Enemy).InjureSkill(b_Attacker, E_BattleHurtType.MAGIC, mSpecialAttack, Id, mCurseHurtValue, b_BattleComponent);
                                break;
                            case E_Identity.Summoned:
                                (mBeAttacker as Summoned).InjureSkill(b_Attacker, E_BattleHurtType.MAGIC, mSpecialAttack, Id, mCurseHurtValue, b_BattleComponent);
                                break;
                            case E_Identity.Pet:
                                (mBeAttacker as Pets).InjureSkill(b_Attacker, E_BattleHurtType.MAGIC, mSpecialAttack, Id, mCurseHurtValue, b_BattleComponent);
                                break;
                            case E_Identity.Hero:
                                (mBeAttacker as GamePlayer).InjureSkill(b_Attacker, E_BattleHurtType.MAGIC, mSpecialAttack, Id, mCurseHurtValue, b_BattleComponent);
                                break;
                            default:
                                break;
                        }
                ...
                // 是否附加毒buffer
                var mCurseResistance = mBeAttacker.GetNumerialFunc(E_GameProperty.CurseResistance);
                if (mCurseResistance == 0)
                {
                    var mCurse225HurtValue = mMinMagicAtteck / 5;
                    if (b_Attacker.BattleMasteryDic.TryGetValue(E_BattleMasteryState.Skill_DuJianStrengthen_251, out var mMasteryValue))
                    {
                        mCurse225HurtValue += mMasteryValue;
                    }
                    mBeAttacker.AddHealthState(E_BattleSkillStats.Curse225, mCurse225HurtValue, mConfig.PersistentTime, 0, mCreateFunc, b_BattleComponent);
                    mBeAttacker.UpdateHealthState();
                }
                else if (mCurseResistance != 255)
                {
                    int mRandomValue = Help_RandomHelper.Range(0, 256);
                    if (mRandomValue % (mCurseResistance + 1) == 0)
                    {
                        var mCurse225HurtValue = mMinMagicAtteck / 5;
                        if (b_Attacker.BattleMasteryDic.TryGetValue(E_BattleMasteryState.Skill_DuJianStrengthen_251, out var mMasteryValue))
                        {
