...
namespace ETHotfix
{
    ...
    /// 
    /// </summary>
    [PropertyNumerialAttribute(BindId = (int)E_GameProperty.MinAtteck)]
    public partial class C_PropertyNumerial_MinAtteck : C_PropertyNumerial
    {
        public override int Run(GamePlayer b_Component, bool b_HasTemporary = true)
        {
            int mResult = 0;
            var mGameProperty = E_GameProperty.MinAtteck;
            ...
            if (b_Component.BattleMasteryDic.TryGetValue(E_BattleMasteryState.BeCommon3027, out var mMasteryValue1))
            {
            ...
            if (ConstServer.PlayerMaster || (b_Component.Data.Level >= 400 && b_Component.Data.OccupationLevel >= 3))
            {
                ...
                switch ((E_GameOccupation)b_Component.Data.PlayerTypeId)
                {
                    ...
                    case E_GameOccupation.Holyteacher:
                        {
                            ...
                            // 最小伤害值 大师增益
                            if (b_Component.BattleMasteryDic.TryGetValue(E_BattleMasteryState.IncreaseMinAttack458, out mMasteryValue))
                            {
                                mResult += mMasteryValue;
                            }
                    ...
                    case E_GameOccupation.Combat:
                        {
                            if (b_Component.BattleMasteryDic.TryGetValue(E_BattleMasteryState.MasterAttribute650, out var mMasteryValue))
                            {
                                mResult += mMasteryValue;
                            }
                            // 物理攻击力提高
                            if (b_Component.BattleMasteryDic.TryGetValue(E_BattleMasteryState.MasterAttribute630, out mMasteryValue))
                            {
                                mResult += mMasteryValue;
                            }
                    ...
                    case E_GameOccupation.GrowLancer:
                        {
                            if (b_Component.BattleMasteryDic.TryGetValue(E_BattleMasteryState.MasterAttribute955, out var mMasteryValue))
                            {
                                mResult += mMasteryValue;
                            }
