...
namespace ETHotfix
{
    ...
    /// 强袭
    /// </summary>
    [HeroSkillAttribute(BindId = (int)E_HeroSkillId.QiangXi)]
    public partial class C_HeroSkill_QiangXi120 : C_HeroSkillSource
    {
    ...
    /// 强袭
    /// </summary>
    public partial class C_HeroSkill_QiangXi120
    {
            ...
            if (mGamePlayer != null)
            {
                ...
                for (int i = 0, len = mKeys.Length; i < len; i++)
                {
                    ...
                    switch (key)
                    {
                        ...
                        case 7:
                            {
                                ...
                                DataCacheManageComponent mDataCacheComponent = mGamePlayer.Player.AddCustomComponent<DataCacheManageComponent>();

                                var mDataCache_Skill = mDataCacheComponent.Get<DBMasterData>();
                                if (mDataCache_Skill == null)
                                {
                                    b_Response.Error = Root.MainFactory.GetCustomComponent<LanguageComponent>().GetMessageId(426);
                                    //b_Response.Message = Root.MainFactory.GetCustomComponent<LanguageComponent>().GetMessage("大师系统等级不足!");
                                    return false;
                                }
                                var mDatalist_Skill = mDataCache_Skill.DataQuery(p => p.GameUserId == mGamePlayer.Player.GameUserId);
                                if (mDatalist_Skill == null || mDatalist_Skill.Count == 0)
                                {
                                    b_Response.Error = Root.MainFactory.GetCustomComponent<LanguageComponent>().GetMessageId(427);
                                    //b_Response.Message = Root.MainFactory.GetCustomComponent<LanguageComponent>().GetMessage("大师系统数据异常!");
                                    return false;
                                }

                                var mData = mDatalist_Skill[0];

                                if (mGamePlayer.MasterGroup.TryGetValue((int)E_BattleMaster_Id.Skill_XiFengCi_skilled132, out var mBattleMaster) == false)
                                {
                                    b_Response.Error = Root.MainFactory.GetCustomComponent<LanguageComponent>().GetMessageId(428);
                                    //b_Response.Message = Root.MainFactory.GetCustomComponent<LanguageComponent>().GetMessage("大师系统技能异常!");
                                    return false;
                                }
                                int mMasterLevel = mData.SkillId[this.Id];
                                if (mBattleMaster.Consume * mMasterLevel < value)
                                {
                                    b_Response.Error = Root.MainFactory.GetCustomComponent<LanguageComponent>().GetMessageId(429);
                                    //b_Response.Message = Root.MainFactory.GetCustomComponent<LanguageComponent>().GetMessage("大师系统技能等级不足!");
                                    return false;
                                }
                            }
                            break;
                        case 8:
                            {
                                if (mGamePlayer.SkillGroup.TryGetValue((int)E_HeroSkillId.XiFengCi, out var mBattleSkill) == false)
                                {
                                    b_Response.Error = Root.MainFactory.GetCustomComponent<LanguageComponent>().GetMessageId(430);
                                    //b_Response.Message = Root.MainFactory.GetCustomComponent<LanguageComponent>().GetMessage("没有学习袭风刺技能!");
                                    return false;
                                }
                            }
                            break;
        ...
        public override bool TryUse(CombatSource b_Attacker, CombatSource b_BeAttacker, C_FindTheWay2D b_Cell, BattleComponent b_BattleComponent, IActorResponse b_Response)
        {
            ...
            int mDistance = mConfig.Distance * 10;
            // 大师技能
            if (b_Attacker.HealthStatsDic.ContainsKey(E_BattleSkillStats.JianZhiFenNu112))
            {
                mDistance = 40;
            }
            if (10 * Vector2.Distance(b_BattleComponent.Parent.GetFindTheWay2D(b_Attacker).Vector2Pos, b_Cell.Vector2Pos) > mDistance)
            {
        ...
        public override bool UseSkill(CombatSource b_Attacker, CombatSource b_BeAttacker, BattleComponent b_BattleComponent)
        {
            ...
            Action<long, long, long> mSyncAction = (b_CombatRoundId, b_AttackerId, b_BeAttackerId) =>
            {
                ...
                int mDistance = mConfig.Distance;
                // 大师技能
                if (b_Attacker.HealthStatsDic.ContainsKey(E_BattleSkillStats.JianZhiFenNu112))
                {
                    mDistance += 1;
                }
                if (10 * Vector2.Distance(mSelfPos, mTargetPos) > mDistance * 10)
                {
                    G2C_AttackResult_notice mAttackResultNotice = new G2C_AttackResult_notice();
                    mAttackResultNotice.HurtValueType = 6;
                    mAttackResultNotice.AttackTarget = mBeAttacker.InstanceId;
                ...
                Vector2 mCenterPos = b_BattleComponent.Parent.GetFindTheWay2D(mBeAttacker).Vector2Pos;
