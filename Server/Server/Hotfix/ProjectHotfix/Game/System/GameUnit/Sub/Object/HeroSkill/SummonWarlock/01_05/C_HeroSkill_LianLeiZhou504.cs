...
namespace ETHotfix
{
    ...
    /// 链雷咒
    /// </summary>
    [HeroSkillAttribute(BindId = (int)E_HeroSkillId.LianLeiZhou504)]
    public partial class C_HeroSkill_LianLeiZhou504 : C_HeroSkillSource
    {
    ...
    /// 链雷咒
    /// </summary>
    public partial class C_HeroSkill_LianLeiZhou504
    {
        ...
        public override bool UseSkill(CombatSource b_Attacker, CombatSource b_BeAttacker, BattleComponent b_BattleComponent)
        {
            ...
            Action<long, long, long> mSyncAction = (b_CombatRoundId, b_AttackerId, b_BeAttackerId) =>
            {
                ...
                int GetHurtValue(E_GameProperty b_SpecialAttack)
                {
                    ...
                    if (b_Attacker.BattleMasteryDic.ContainsKey(E_BattleMasteryState.Skill_LianLeiZou_Strengthen526))
                    {
                        int mMasteryValue = b_Attacker.BattleMasteryDic[E_BattleMasteryState.Skill_LianLeiZou_Strengthen526];
                ...
                // 最大伤害个数
                int mMaxHitCount = 3;
                float[] mHurtValueIncrease = new float[4] { 1, 0.5f, 0.7f, 1f };
                        ...
                        for (int j = 0, jlen = mFieldEnemylist.Length; j < jlen; j++)
                        {
                            ...
                            //if (Vector2.Distance(mSelfPos, new Vector2(mCurrentTemp.UnitData.X, mCurrentTemp.UnitData.Y)) < mRadius)
                            {
                                ...
                                if (IsHit)
                                {
                                    ...
                                    int mRealHurtValue = (int)(mHurtValue * mHurtValueIncrease[mMaxHitCount]);
                                    mCurrentTemp.InjureSkill(b_Attacker, E_BattleHurtType.MAGIC, mSpecialAttack, Id, mRealHurtValue, b_BattleComponent);
                                    mMaxHitCount--;
                                }
                                else
                                {
