...
namespace ETHotfix
{
    ...
    /// 天雷闪
    /// </summary>
    [HeroSkillAttribute(BindId = (int)E_HeroSkillId.TianLeiShan327)]
    public partial class C_HeroSkill_TianLeiShan327 : C_HeroSkillSource
    {
    ...
    /// 天雷闪
    /// </summary>
    public partial class C_HeroSkill_TianLeiShan327
    {
        ...
        public override bool UseSkill(CombatSource b_Attacker, CombatSource b_BeAttacker, BattleComponent b_BattleComponent)
        {
            ...
            Action<long, long, long> mSyncAction = (b_CombatRoundId, b_AttackerId, b_BeAttackerId) =>
            {
                ...
                //Vector2 mViewDirection = (mTargetPos - mSelfPos).normalized;
                //float mViewAngle = 180 * 0.5f;

                //int mTargetX = b_Attacker.UnitData.X;
                //int mTargetY = b_Attacker.UnitData.Y;
                //Vector2 mCenterPos = mSelfPos;
                //Dictionary<long, C_FindTheWay2D> mMapFieldDic = new Dictionary<long, C_FindTheWay2D>();
                //var mRadius = 4;
                ...
                //        Vector2 mCurrentTempPos = mCurrentTemp.Vector2Pos;
                //        if (10 * Vector2.Distance(mCenterPos, mCurrentTempPos) >= mRadiusLen) continue;

                //        Vector2 mViewDirectionTemp = (mCurrentTempPos - mSelfPos).normalized;
                //        float angle = Vector2.Angle(mViewDirectionTemp, mViewDirection);
                //        if (angle >= mViewAngle) continue;
                ...
                int GetHurtValue(E_GameProperty b_SpecialAttack)
                {
                    ...
                    // 大师技能 霹雳回旋斩强化
                    if (b_Attacker.BattleMasteryDic.ContainsKey(E_BattleMasteryState.Skill_TianLeiShanStrengthen329))
                    {
                        int mMasteryValue = b_Attacker.BattleMasteryDic[E_BattleMasteryState.Skill_TianLeiShanStrengthen329];
