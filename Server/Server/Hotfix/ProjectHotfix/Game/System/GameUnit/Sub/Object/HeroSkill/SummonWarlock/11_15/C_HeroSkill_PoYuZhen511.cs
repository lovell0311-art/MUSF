...
namespace ETHotfix
{
    ...
    /// 破御阵
    /// </summary>
    [HeroSkillAttribute(BindId = (int)E_HeroSkillId.PoYuZhen511)]
    public partial class C_HeroSkill_PoYuZhen511 : C_HeroSkillSource
    {
    ...
    /// 破御阵
    /// </summary>
    public partial class C_HeroSkill_PoYuZhen511
    {
                ...
                void Caculation(CombatSource b_CurrentTemp)
                {
                    switch (b_CurrentTemp.Identity)
                    {
                        ...
                        case E_Identity.Pet:
                            {
                                mHitValue = 12 + mWillpower / 110;
                                int mLevel = (b_Attacker as GamePlayer).Data.Level - (b_CurrentTemp as Pets).dBPetsData.PetsLevel;
                                mPersistentTime = 5 + mWillpower / 300 - mLevel / 150;
                            }
                            break;
                        case E_Identity.Summoned:
                    ...
                    if (b_Attacker.BattleMasteryDic.ContainsKey(E_BattleMasteryState.Skill_PoYuZhen_Strengthen536))
                    {
                        int mMasteryValue = b_Attacker.BattleMasteryDic[E_BattleMasteryState.Skill_PoYuZhen_Strengthen536];
                        mHitValue += mMasteryValue;

                        var mMasterSkilllevel = (int)E_BattleMaster_Id.Skill_PoYuZhen_Strengthen536;
                    ...
                    if (b_Attacker.BattleMasteryDic.ContainsKey(E_BattleMasteryState.Skill_PoYuZhen_Master539))
                    {
                        int mMasteryValue = b_Attacker.BattleMasteryDic[E_BattleMasteryState.Skill_PoYuZhen_Master539];
                        mHitValue += mMasteryValue;
                    }
                    ...
                    if (mPersistentTime > 1000)
                    {
                        ...
                        b_CurrentTemp.ChangeNumerialType(E_GameProperty.Defense);
                    }
                }
