...
namespace ETHotfix
{
    ...
    /// 牙突刺
    /// </summary>
    [HeroSkillAttribute(BindId = (int)E_HeroSkillId.YaTuCi701)]
    public partial class C_HeroSkill_YaTuCi701 : C_HeroSkillSource
    {
    ...
    /// 牙突刺
    /// </summary>
    public partial class C_HeroSkill_YaTuCi701
    {
        ...
        public override bool UseSkill(CombatSource b_Attacker, CombatSource b_BeAttacker, BattleComponent b_BattleComponent)
        {
            ...
            Action<long, long, long> mSyncAction = (b_CombatRoundId, b_AttackerId, b_BeAttackerId) =>
            {
                ...
                if (xAxial == false || yAxial == false)
                {
                    switch (b_Attacker.Identity)
                    {
                        case E_Identity.Hero:
                            {
                                var mGamePlayer = b_Attacker as GamePlayer;
                                // 公告移动信息
                                b_BattleComponent.Parent.MoveSendNotice(mFindTheWaySource, b_Cell, mGamePlayer, false);
                        ...
                        case E_Identity.Enemy:
                            {
                                // 公告移动信息
                                b_BattleComponent.Parent.MoveSendNotice(mFindTheWaySource, b_Cell, b_Attacker as Enemy, false);
                            }
                            break;
                        case E_Identity.Npc:
                            break;
                        case E_Identity.Pet:
                            {
                                // 公告移动信息
                                b_BattleComponent.Parent.MoveSendNotice(mFindTheWaySource, b_Cell, b_Attacker as Pets, false);
                            }
                            break;
                        case E_Identity.Summoned:
                            {
                                // 公告移动信息
                                b_BattleComponent.Parent.MoveSendNotice(mFindTheWaySource, b_Cell, b_Attacker as Summoned, false);
                            }
                            break;
                        default:
                            break;
                ...
                void JiTui()
                {
                    ...
                    if (mConfig.OtherDataDic.TryGetValue(4, out int mRepelValue) && mRepelValue > 0)
                    {
                        ...
                        switch (mBeAttacker.Identity)
                        {
                            ...
                            case E_Identity.Enemy:
                                {
                                    if ((mBeAttacker as Enemy).IsCheckState(3) == false)
                                        // 公告移动信息
                ...
                int mMasteryValue = 0;
                if (b_Attacker.BattleMasteryDic.ContainsKey(E_BattleMasteryState.MasterAttribute925))
                    mMasteryValue = b_Attacker.BattleMasteryDic[E_BattleMasteryState.MasterAttribute925];
               
                if (mSpecialAttack == E_GameProperty.LucklyAttackRate)
                {
                    mVirtualHurtValue = mConfig.OtherDataDic[1] + b_Attacker.GetNumerialFunc(E_GameProperty.MaxAtteck) + b_Attacker.GetNumerialFunc(E_GameProperty.DreamRiderIrritate) + b_Attacker.GetNumerialFunc(E_GameProperty.DreamRiderPenalize);
                    mVirtualHurtValue += mMasteryValue;
                }
                else
                {
                    int mMinMagicAtteck = b_Attacker.GetNumerialFunc(E_GameProperty.MinAtteck) + b_Attacker.GetNumerialFunc(E_GameProperty.DreamRiderIrritate) + b_Attacker.GetNumerialFunc(E_GameProperty.DreamRiderPenalize);
                    int mMaxMagicAtteck = b_Attacker.GetNumerialFunc(E_GameProperty.MaxAtteck) + b_Attacker.GetNumerialFunc(E_GameProperty.DreamRiderIrritate) + b_Attacker.GetNumerialFunc(E_GameProperty.DreamRiderPenalize);
                    mVirtualHurtValue = mConfig.OtherDataDic[1] + Help_RandomHelper.Range(mMinMagicAtteck+ mMasteryValue, mMaxMagicAtteck+ mMasteryValue);
                }

                var mStrength = b_Attacker.GetNumerialFunc(E_GameProperty.Property_Strength);

                // 总伤害 = 魔法伤害 + 魔力伤害 + 魔杖魔力提升百分比
                int mHurtValue = mVirtualHurtValue + mStrength / mConfig.OtherDataDic[10];
