...
namespace ETHotfix
{
    ...
    /// 狂怒
    /// </summary>
    [HeroSkillAttribute(BindId = (int)E_HeroSkillId.KuangNu709)]
    public partial class C_HeroSkill_KuangNu709 : C_HeroSkillSource
    {
        ...
        public override int GetCoolTime(CombatSource b_Attacker)
        {
            if (b_Attacker.BattleMasteryDic.TryGetValue(E_BattleMasteryState.MasterAttribute954,out var Value))
            {
                return CoolTime - Value < 0 ? 0 : CoolTime - Value;
            }
            return CoolTime;
        }
    }
    ...
    /// 狂怒
    /// </summary>
    public partial class C_HeroSkill_KuangNu709
    {
            ...
            Action<long, long, long> mSyncAction = (b_CombatRoundId, b_AttackerId, b_BeAttackerId) =>
            {
                ...
                Func<CombatSource.BattleSyncTimerTask> mCreateFunc = () =>
                {
                    ...
                    mBattleSyncTimer.DisposeAction = (b_CombatSource, b_BattleComponent, b_TimerTask) =>
                    {
                        ...
                        b_CombatSource.RemoveHealthState(E_BattleSkillStats.KuangNu709, b_BattleComponent);
                    ...
                    mBattleSyncTimer.SyncAction = (b_CombatSource, b_BattleComponent, b_TimerTask, b_TimeTick) =>
                    {
                        ...
                        if (b_CombatSource.HealthStatsDic.TryGetValue(E_BattleSkillStats.KuangNu709, out var hp_Curse) == false)
                        {
                ...
                if (!b_Attacker.HealthStatsDic.ContainsKey(E_BattleSkillStats.KuangNu709))
                {
                    int b_AddValue = 0;
                    b_AddValue = b_Attacker.GetNumerialFunc(E_GameProperty.MinAtteck);
                    b_AddValue = (int)(b_AddValue * 0.3);
                    int PersistentTime = mConfig.PersistentTime;
                    if (b_Attacker.BattleMasteryDic.ContainsKey(E_BattleMasteryState.MasterAttribute950))
                    {
                        int mMasteryValue = b_Attacker.BattleMasteryDic[E_BattleMasteryState.MasterAttribute950];
                        PersistentTime += mMasteryValue;
                    }
                    b_Attacker.AddHealthState(E_BattleSkillStats.KuangNu709, b_AddValue, PersistentTime, 0, mCreateFunc, b_BattleComponent);
