...
namespace ETHotfix
{
    ...
    /// 玄月斩
    /// </summary>
    [HeroSkillAttribute(BindId = (int)E_HeroSkillId.XuanYueZhan321)]
    public partial class C_HeroSkill_XuanYueZhan321 : C_HeroSkillSource
    {
    ...
    /// 玄月斩
    /// </summary>
    public partial class C_HeroSkill_XuanYueZhan321
    {
            ...
            Action<long, long, long> mSyncAction = (b_CombatRoundId, b_AttackerId, b_BeAttackerId) =>
            {
                ...
                Action<long, long, long> mSyncActionChild = (b_CombatRoundId, b_AttackerId, b_BeAttackerId) =>
                {
                    ...
                    int GetHurtValue(E_GameProperty b_SpecialAttack)
                    {
                        ...
                        // 大师技能 霹雳回旋斩强化
                        if (b_Attacker.BattleMasteryDic.ContainsKey(E_BattleMasteryState.Skill_XuanYueZhanStrengthen333))
                        {
                            int mMasteryValue = b_Attacker.BattleMasteryDic[E_BattleMasteryState.Skill_XuanYueZhanStrengthen333];
                    ...
                    if (b_Attacker.BattleMasteryDic.ContainsKey(E_BattleMasteryState.Skill_XuanYueZhanMaster337))
                    {
                        int mMasteryValue = b_Attacker.BattleMasteryDic[E_BattleMasteryState.Skill_XuanYueZhanMaster337];
                        mBuffValue += mMasteryValue;
                    }
                    Func<CombatSource.BattleSyncTimerTask> mCreateFunc = () =>
                    {
                        ...
                        mBattleSyncTimer.DisposeAction = (b_CombatSource, b_BattleComponent, b_TimerTask) =>
                        {
                            ...
                            b_CombatSource.RemoveHealthState(E_BattleSkillStats.XuanYueZhan321, b_BattleComponent);
                        ...
                        mBattleSyncTimer.SyncAction = (b_CombatSource, b_BattleComponent, b_TimerTask, b_TimeTick) =>
                        {
                            ...
                            if (b_CombatSource.HealthStatsDic.TryGetValue(E_BattleSkillStats.XuanYueZhan321, out var hp_Curse) == false)
                            {
                            ...
                            for (int j = 0, jlen = mFieldEnemylist.Length; j < jlen; j++)
                            {
                                ...
                                //if (Vector2.Distance(mSelfPos, new Vector2(mCurrentTemp.UnitData.X, mCurrentTemp.UnitData.Y)) < mConfig.Distance)
                                {
                                    ...
                                    if (IsHit)
                                    {
                                        ...
                                        mCurrentTemp.AddHealthState(E_BattleSkillStats.XuanYueZhan321, mBuffValue, mConfig.PersistentTime, 0, mCreateFunc, b_BattleComponent);
                                        mCurrentTemp.UpdateHealthState();
                                        mCurrentTemp.ChangeNumerialType(E_GameProperty.Defense);
                                    }
                                    else
                                    {
                                        G2C_AttackResult_notice mAttackResultNotice = new G2C_AttackResult_notice();
                ...
                b_BattleComponent.WaitSync(1000, b_AttackerId, b_BeAttackerId, mSyncActionChild);
            };
