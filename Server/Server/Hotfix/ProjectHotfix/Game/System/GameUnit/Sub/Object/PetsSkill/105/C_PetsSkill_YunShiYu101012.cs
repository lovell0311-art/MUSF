...
namespace ETHotfix
{
    ...
    /// 黑龙波
    /// </summary>
    [HeroSkillAttribute(BindId = (int)E_Pets_SKILL_Id.YunShiYu)]
    public partial class C_PetsSkill_YunShiYu101012 : C_HeroSkillSource
    {
    ...
    /// 萝卜雨
    /// </summary>
    public partial class C_PetsSkill_YunShiYu101012
    {
        ...
        public override bool UseSkill(CombatSource b_Attacker, CombatSource b_BeAttacker, BattleComponent b_BattleComponent)
        {
            ...
            mAttackTime += mConfig.SkillInterval;
            ...
            Action<long, long, long> mSyncAction = (b_CombatRoundId, b_AttackerId, b_BeAttackerId) =>
            {
                ...
                Action<long, long, long> mSyncActionChild = (b_CombatRoundId, b_AttackerId, b_BeAttackerId) =>
                {
                    ...
                    if (instanceId != b_Attacker.Id) return;
                    int mMinAtteck = mConfig.OtherDataDic[1] + b_Attacker.GetNumerialFunc(E_GameProperty.MinMagicAtteck);
                    int mMaxAtteck = (int)(1.5 *mConfig.OtherDataDic[1] )+ b_Attacker.GetNumerialFunc(E_GameProperty.MaxMagicAtteck);
                    ...
                    long PetsPlayerID = b_Attacker.GetPlayerInstance();
                    ...
                    for (int i = 0, len = mMapFieldlist.Length; i < len; i++)
                    {
                        ...
                        if (mMapField.FieldPlayerDic.Count > 0)
                        {
                            ...
                            for (int j = 0, jlen = mFieldPlayerlist.Length; j < jlen; j++)
                            {
                                ...
                                if (mCurrentTemp.IsDeath || mCurrentTemp.IsDisposeable) continue;
                                if (mCurrentTemp.InstanceId == PetsPlayerID) continue;
                        ...
                        if (false && mMapField.FieldPetsDic.Count > 0)
                        {
                            ...
                            for (int j = 0, jlen = mFieldPetslist.Length; j < jlen; j++)
                            {
                                ...
                                if (mCurrentTemp.IsDeath || mCurrentTemp.IsDisposeable) continue;
                                if (mCurrentTemp.GamePlayer.InstanceId == PetsPlayerID) continue;
