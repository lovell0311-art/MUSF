...
namespace ETHotfix
{
    ...
    /// 袭风刺强化
    /// </summary>
    [BattleMasterAttribute(BindId = (int)E_BattleMaster_Id.Skill_XiFengCi129)]
    public partial class C_BattleMaster_Skill_XiFengCi129 : C_BattleMaster
    {
    ...
    /// 袭风刺强化
    /// </summary>
    public partial class C_BattleMaster_Skill_XiFengCi129
    {
        ...
        public override bool UseSkill(GamePlayer b_Attacker, BattleComponent b_BattleComponent, bool b_AddPoint)
        {
            ...
            // 使用坚固的庇护时HP转换率追加增加{0:F}%。

            int mMasterLevel = Data.SkillId[this.Id];
            b_Attacker.BattleMasteryDic[E_BattleMasteryState.Skill_XiFengCi129] = base.GetMasterValue(mConfig.OtherDataDic[mMasterLevel], mConfig.OtherDataUse);

            if (b_AddPoint)
            {
                if (mMasterLevel == 1)
                {
                    int mAttackType = (int)E_HeroSkillId.XiFengCi;

                    var mJsonDic = Root.MainFactory.GetCustomComponent<CustomFrameWork.Component.ReadConfigComponent>().GetJson<Skill_SwordsmanConfigJson>().JsonDic;

                    if (mJsonDic.TryGetValue(mAttackType, out var mJson))
                    {
                        G2C_AttackDistance_notice mMessage = new G2C_AttackDistance_notice();
                        mMessage.AttackType = mAttackType;
                        //mMessage.Distance = mJson.Distance + 4;
                        mMessage.Distance = 4;
                        b_Attacker.Player.Send(mMessage);
                    }
                }
            }
            else
