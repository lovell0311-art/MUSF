...
namespace ETHotfix
{
    ...
    /// 提高最大SD
    /// </summary>
    [BattleMasterAttribute(BindId = (int)E_BattleMaster_Id.MasterAttribute903)]
    public partial class C_BattleMaster_MasterAttribute903 : C_BattleMaster
    {
        ...
        {  //数据变化 更新变更数据 
            ...
            if (!(Config is BattleMaster_DreamKnightConfig mConfig))
            {
                return;
            }
    ...
    /// 提高最大SD
    /// </summary>
    public partial class C_BattleMaster_MasterAttribute903
    {
        public override bool TryUse(GamePlayer b_Attacker, BattleComponent b_BattleComponent, IActorResponse b_Response)
        {
            ...
            if (mConfig.FrontIds != null && mConfig.FrontIds.Count > 0)
            {
                ...
                for (int i = 0, len = mConfig.FrontIds.Count; i < len; i++)
                {
                    ...
                    if (mReadConfigComponent.GetJson<BattleMaster_DreamKnightConfigJson>().JsonDic.TryGetValue(mLastId, out var mTargetConfig) == false)
                    {
        ...
        public override bool UseSkill(GamePlayer b_Attacker, BattleComponent b_BattleComponent, bool b_AddPoint)
        {
            ...
            b_Attacker.BattleMasteryDic[E_BattleMasteryState.MasterAttribute903] = base.GetMasterValue(mConfig.OtherDataDic[mMasterLevel], mConfig.OtherDataUse);
            if (b_AddPoint)
            {
