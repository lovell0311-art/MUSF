...
namespace ETHotfix
{
    ...
    /// 守护之光
    /// </summary>
    [HeroSkillAttribute(BindId = (int)E_HeroSkillId.ShouHuZhiGuang)]
    public partial class C_HeroSkill_ShouHuZhiGuang203 : C_HeroSkillSource
    {
    ...
    /// 守护之光
    /// </summary>
    public partial class C_HeroSkill_ShouHuZhiGuang203
    {
        ...
        public override bool UseSkill(CombatSource b_Attacker, CombatSource b_BeAttacker, BattleComponent b_BattleComponent)
        {
            ...
            Action<long, long, long> mSyncAction = (b_CombatRoundId, b_AttackerId, b_BeAttackerId) =>
            {
                ...
                {
                    // 提升防御力=2+智力/8
                    // 如果是剑士，提升防御力 = (2 + 智力 / 8) * 1.1，其他职业不乘1.1
                    var mWillpower = b_Attacker.GetNumerialFunc(E_GameProperty.Property_Willpower);
                    int mStrengthValue = 2 + (int)(mWillpower / 9);

                    if (b_Attacker.BattleMasteryDic.TryGetValue(E_BattleMasteryState.Skill_DefenseIncreaseStrengthen229, out var mMasteryValue))
                    {
                        mStrengthValue = (int)(mStrengthValue * (100 + mMasteryValue) / 100f);
                    }
                    if (b_Attacker.BattleMasteryDic.TryGetValue(E_BattleMasteryState.Skill_DefenseIncreaseMaster236, out mMasteryValue))
                    {
                        mStrengthValue = (int)(mStrengthValue * (100 + mMasteryValue) / 100f);
                    }
                    ...
                    Func<CombatSource.BattleSyncTimerTask> mCreateFunc = () =>
                    {
                        ...
                        mBattleSyncTimer.DisposeAction = (b_CombatSource, b_BattleComponent, b_TimerTask) =>
                        {
                            ...
                            b_CombatSource.RemoveHealthState(E_BattleSkillStats.ShouHuZhiGuang203, b_BattleComponent);
                        ...
                        mBattleSyncTimer.SyncAction = (b_CombatSource, b_BattleComponent, b_TimerTask, b_TimeTick) =>
                        {
                            ...
                            if (b_CombatSource.HealthStatsDic.TryGetValue(E_BattleSkillStats.ShouHuZhiGuang203, out var hp_Curse) == false)
                            {
                    ...
                    void action(CombatSource b_BeAttacker)
                    {
                        ...
                        // 提高防御力
                        b_BeAttacker.AddHealthState(E_BattleSkillStats.ShouHuZhiGuang203, b_AddValue, mConfig.PersistentTime, 0, mCreateFunc, b_BattleComponent);
                        b_BeAttacker.UpdateHealthState();
                        b_BeAttacker.ChangeNumerialType(E_GameProperty.Defense);
                    }
                    action(b_Attacker);
                    {
                        var mGamePlayer = b_Attacker as GamePlayer;
