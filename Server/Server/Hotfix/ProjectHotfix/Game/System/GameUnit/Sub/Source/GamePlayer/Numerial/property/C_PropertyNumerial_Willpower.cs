...
namespace ETHotfix
{
    ...
    /// 
    /// </summary>
    [PropertyNumerialAttribute(BindId = (int)E_GameProperty.Property_Willpower)]
    public partial class C_PropertyNumerial_Willpower : C_PropertyNumerial
    {
            ...
            if (b_Component.Data.Level >= 400 && b_Component.Data.OccupationLevel >= 3)
            {
                switch ((E_GameOccupation)b_Component.Data.PlayerTypeId)
                {
                    ...
                    case E_GameOccupation.Combat:
                        {
                            if (b_Component.BattleMasteryDic.TryGetValue(E_BattleMasteryState.MasterAttribute615, out var mMasteryValue))
                            {
                                mResult += mMasteryValue;
                            }
                        }
                    ...
                    case E_GameOccupation.GrowLancer:
                        {
                            if (b_Component.BattleMasteryDic.TryGetValue(E_BattleMasteryState.MasterAttribute915, out var mMasteryValue))
                            {
                                mResult += mMasteryValue;
                            }
                        }
