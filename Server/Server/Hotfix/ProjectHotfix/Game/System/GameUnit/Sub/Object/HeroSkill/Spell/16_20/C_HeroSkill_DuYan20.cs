...
namespace ETHotfix
{
    ...
    /// 毒炎
    /// </summary>
    [HeroSkillAttribute(BindId = (int)E_HeroSkillId.DuYan)]
    public partial class C_HeroSkill_DuYan20 : C_HeroSkillSource
    {
    ...
    /// 毒炎
    /// </summary>
    public partial class C_HeroSkill_DuYan20
    {
            ...
            Action<long, long, long> mSyncAction = (b_CombatRoundId, b_AttackerId, b_BeAttackerId) =>
            {
                ...
                int GetHurtValue(E_GameProperty b_SpecialAttack)
                {
                    ...
                    else
                    {
                        ...
                        mVirtualHurtValue += b_Attacker.GetNumerialFunc(E_GameProperty.EmbedGreatShotAttack) + b_Attacker.GetNumerialFunc(E_GameProperty.EmbedSkillAttack);
                    }
                    ...
                    if (b_Attacker.BattleMasteryDic.ContainsKey(E_BattleMasteryState.Skill_DuYanStrengthen34))
                    {
                        int mMasteryValue = b_Attacker.BattleMasteryDic[E_BattleMasteryState.Skill_DuYanStrengthen34];
                ...
                Func<CombatSource.BattleSyncTimerTask> mCreateFunc = () =>
                {
                    ...
                    mBattleSyncTimer.DisposeAction = (b_CombatSource, b_BattleComponent, b_TimerTask) =>
                    {
                        ...
                        b_CombatSource.RemoveHealthState(E_BattleSkillStats.Curse, b_BattleComponent);
                    ...
                    mBattleSyncTimer.SyncAction = (b_CombatSource, b_BattleComponent, b_TimerTask, b_TimeTick) =>
                    {
                        ...
                        if (b_CombatSource.HealthStatsDic.TryGetValue(E_BattleSkillStats.Curse, out var hp_Curse) == false)
                        {
                            return CombatSource.E_SyncTimerTaskResult.Dispose;
                        }

                        if (hp_Curse.CacheDatas.TryGetValue(0, out var mCurseData) == false)
                        {
                            return CombatSource.E_SyncTimerTaskResult.Dispose;
                        }

                        var mCurseHurtValue = mCurseData.StrengthValue;

                        switch (mBeAttacker.Identity)
                        {
                            case E_Identity.Enemy:
                                (mBeAttacker as Enemy).InjureSkill(b_Attacker, E_BattleHurtType.MAGIC, E_GameProperty.NullAttack, Id, mCurseHurtValue, b_BattleComponent);
                                break;
                            case E_Identity.Summoned:
                                (mBeAttacker as Summoned).InjureSkill(b_Attacker, E_BattleHurtType.MAGIC, E_GameProperty.NullAttack, Id, mCurseHurtValue, b_BattleComponent);
                                break;
                            case E_Identity.Pet:
                                (mBeAttacker as Pets).InjureSkill(b_Attacker, E_BattleHurtType.MAGIC, E_GameProperty.NullAttack, Id, mCurseHurtValue, b_BattleComponent);
                                break;
                            case E_Identity.Hero:
                                (mBeAttacker as GamePlayer).InjureSkill(b_Attacker, E_BattleHurtType.MAGIC, E_GameProperty.NullAttack, Id, mCurseHurtValue, b_BattleComponent);
                                break;
                            default:
                                break;
                        }
                        ...
                        for (int j = 0, jlen = mFieldEnemylist.Length; j < jlen; j++)
                        {
                            ...
                            //if (Vector2.Distance(mSelfPos, new Vector2(mCurrentTemp.UnitData.X, mCurrentTemp.UnitData.Y)) < mRadius)
                            {
                                ...
                                if (IsHit)
                                {
                                    ...
                                    // 是否附加毒buffer
                                    var mCurseResistance = mCurrentTemp.GetNumerialFunc(E_GameProperty.CurseResistance);
                                    if (mCurseResistance == 0)
                                    {
                                        mCurrentTemp.AddHealthState(E_BattleSkillStats.Curse, (int)(mHurtValue * 0.5f), mConfig.PersistentTime, 0, mCreateFunc, b_BattleComponent);
