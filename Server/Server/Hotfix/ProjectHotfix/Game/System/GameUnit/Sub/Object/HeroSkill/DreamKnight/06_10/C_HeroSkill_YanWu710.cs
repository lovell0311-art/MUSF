...
namespace ETHotfix
{
    ...
    /// 炎舞
    /// </summary>
    [HeroSkillAttribute(BindId = (int)E_HeroSkillId.YanWu710)]
    public partial class C_HeroSkill_YanWu710 : C_HeroSkillSource
    {
    ...
    /// 炎舞
    /// </summary>
    public partial class C_HeroSkill_YanWu710
    {
        ...
        public override bool TryUse(CombatSource b_Attacker, CombatSource b_BeAttacker, C_FindTheWay2D b_Cell, BattleComponent b_BattleComponent, IActorResponse b_Response)
        {
            ...
            int Distance = mConfig.Distance;
            if (b_Attacker.BattleMasteryDic.ContainsKey(E_BattleMasteryState.MasterAttribute939))
            {
                int mMasteryValue = b_Attacker.BattleMasteryDic[E_BattleMasteryState.MasterAttribute939];
                Distance += mMasteryValue;
            }
            if (Vector2.Distance(b_BattleComponent.Parent.GetFindTheWay2D(b_Attacker).Vector2Pos, b_Cell.Vector2Pos) > Distance)
            {
        ...
        public override bool UseSkill(CombatSource b_Attacker, CombatSource b_BeAttacker, BattleComponent b_BattleComponent)
        {
            ...
            Action<long, long, long> mSyncAction = (b_CombatRoundId, b_AttackerId, b_BeAttackerId) =>
            {
                ...
                if (Vector2.Distance(mSelfPos, mTargetPos) > Distance)
                {
                    G2C_AttackResult_notice mAttackResultNotice = new G2C_AttackResult_notice();
                    mAttackResultNotice.HurtValueType = 6;
                    mAttackResultNotice.AttackTarget = mBeAttacker.InstanceId;
                ...
                int mMinAtteck = mConfig.OtherDataDic[1] + b_Attacker.GetNumerialFunc(E_GameProperty.MinAtteck) + b_Attacker.GetNumerialFunc(E_GameProperty.DreamRiderIrritate) + b_Attacker.GetNumerialFunc(E_GameProperty.DreamRiderPenalize);
                int mMaxAtteck = mConfig.OtherDataDic[1] + b_Attacker.GetNumerialFunc(E_GameProperty.MaxAtteck) + b_Attacker.GetNumerialFunc(E_GameProperty.DreamRiderIrritate) + b_Attacker.GetNumerialFunc(E_GameProperty.DreamRiderPenalize);
                ...
                if (b_Attacker.BattleMasteryDic.ContainsKey(E_BattleMasteryState.MasterAttribute934))
                {
                    int mMasteryValue = b_Attacker.BattleMasteryDic[E_BattleMasteryState.MasterAttribute934];
