...
namespace ETHotfix
{
    ...
    /// 提高最大SD
    /// </summary>
    [BattleMasterAttribute(BindId = (int)E_BattleMaster_Id.IncreaseSDMax3)]
    public partial class C_BattleMaster_IncreaseSDMax3 : C_BattleMaster
    {
        ...
        {  //数据变化 更新变更数据 
            ...
            if (!(Config is BattleMaster_SpellConfig mConfig))
            {
                return;
            }
    ...
    /// 提高最大SD
    /// </summary>
    public partial class C_BattleMaster_IncreaseSDMax3
    {
        public override bool TryUse(GamePlayer b_Attacker, BattleComponent b_BattleComponent, IActorResponse b_Response)
        {
            ...
            if (mConfig.FrontIds != null && mConfig.FrontIds.Count > 0)
            {
                ...
                for (int i = 0, len = mConfig.FrontIds.Count; i < len; i++)
                {
                    ...
                    if (mReadConfigComponent.GetJson<BattleMaster_SpellConfigJson>().JsonDic.TryGetValue(mLastId, out var mTargetConfig) == false)
                    {
        ...
        public override bool UseSkill(GamePlayer b_Attacker, BattleComponent b_BattleComponent, bool b_AddPoint)
        {
            ...
            b_Attacker.BattleMasteryDic[E_BattleMasteryState.IncreaseSDMax3] = base.GetMasterValue(mConfig.OtherDataDic[mMasterLevel], mConfig.OtherDataUse);
            b_Attacker.ChangeNumerialType(E_GameProperty.PROP_SD_MAX);

            if (b_AddPoint)
            {
                var mGameProperty = E_GameProperty.PROP_SD_MAX;
                ...
                mBattleKVData.Value = b_Attacker.GetNumerial(mGameProperty);
                mChangeValue_notice.Info.Add(mBattleKVData);
                b_Attacker.Player.Send(mChangeValue_notice);
            }
