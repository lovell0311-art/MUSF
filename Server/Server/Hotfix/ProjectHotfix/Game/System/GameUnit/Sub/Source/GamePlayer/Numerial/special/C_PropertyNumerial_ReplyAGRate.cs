...
namespace ETHotfix
{
    ...
    /// 
    /// </summary>
    [PropertyNumerialAttribute(BindId = (int)E_GameProperty.ReplyAGRate)]
    public partial class C_PropertyNumerial_ReplyAGRate : C_PropertyNumerial
    {
        public override int Run(GamePlayer b_Component, bool b_HasTemporary = true)
        {
            ...
            if (b_HasTemporary)
            {
                if (b_Component.HealthStatsDic.TryGetValue(E_BattleSkillStats.Use310135, out var mTempBuffer))
                {
            ...
            if (b_Component.BattleMasteryDic.TryGetValue(E_BattleMasteryState.BeCommon3010, out var mMasteryValue1))
            {
            ...
            if (b_Component.Data.Level >= 400 && b_Component.Data.OccupationLevel >= 3)
            {
                switch ((E_GameOccupation)b_Component.Data.PlayerTypeId)
                {
                    ...
                    case E_GameOccupation.Combat:
                        {
                            if (b_Component.BattleMasteryDic.TryGetValue(E_BattleMasteryState.MasterAttribute610, out var mMasteryValue))
                            {
                                mResult += mMasteryValue;
                            }
                        }
                    ...
                    case E_GameOccupation.GrowLancer:
                        {
                            if (b_Component.BattleMasteryDic.TryGetValue(E_BattleMasteryState.MasterAttribute910, out var mMasteryValue))
                            {
                                mResult += mMasteryValue;
                            }
                        }
