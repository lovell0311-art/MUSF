...
namespace ETHotfix
{
    ...
    /// 神圣气旋
    /// </summary>
    [HeroSkillAttribute(BindId = (int)E_HeroSkillId.ShenShengQiXuan611)]
    public partial class C_HeroSkill_ShenShengQiXuan611 : C_HeroSkillSource
    {
    ...
    /// 神圣气旋
    /// </summary>
    public partial class C_HeroSkill_ShenShengQiXuan611
    {
        ...
        public override bool UseSkill(CombatSource b_Attacker, CombatSource b_BeAttacker, BattleComponent b_BattleComponent)
        {
            ...
            Action<long, long, long> mSyncAction = (b_CombatRoundId, b_AttackerId, b_BeAttackerId) =>
            {
                ...
                Vector2 mSelfPos = mFindTheWaySource.Vector2Pos;
                ...
                if (b_Attacker.BattleMasteryDic.ContainsKey(E_BattleMasteryState.MasterAttribute625))
                {
                    int mMasteryValue = b_Attacker.BattleMasteryDic[E_BattleMasteryState.MasterAttribute625];
                    mMaxAtteck += mMasteryValue;
                    mMinAtteck += mMasteryValue;
                }
                int mRate = 20;
                if (b_Attacker.BattleMasteryDic.ContainsKey(E_BattleMasteryState.MasterAttribute628))
                {
                    int mMasteryValue = b_Attacker.BattleMasteryDic[E_BattleMasteryState.MasterAttribute628];
                    mRate += mMasteryValue;
                }
                ...
                Func<CombatSource.BattleSyncTimerTask> mCreateFunc = () =>
                {
                    ...
                    mBattleSyncTimer.DisposeAction = (b_CombatSource, b_BattleComponent, b_TimerTask) =>
                    {
                        ...
                        b_CombatSource.RemoveHealthState(E_BattleSkillStats.ShenShengQiXuan611, b_BattleComponent);
                    ...
                    mBattleSyncTimer.SyncAction = (b_CombatSource, b_BattleComponent, b_TimerTask, b_TimeTick) =>
                    {
                        ...
                        if (b_CombatSource.HealthStatsDic.TryGetValue(E_BattleSkillStats.ShenShengQiXuan611, out var hp_Curse) == false)
                        {
                ...
                for (int i = 0, len = mMapFieldlist.Length; i < len; i++)
                {
                    ...
                    if (mMapField.FieldEnemyDic.Count > 0)
                    {
                        ...
                        for (int j = 0, jlen = mFieldEnemylist.Length; j < jlen; j++)
                        {
                            ...
                            var mRandomIndex = Help_RandomHelper.Range(0, 100);
                            if (mRandomIndex < 20)
                            {
                                JiTui(mCurrentTemp);

                                mCurrentTemp.AddHealthState(E_BattleSkillStats.ShenShengQiXuan611, 200, mConfig.PersistentTime, 0, mCreateFunc, b_BattleComponent);
                ...
                void JiTui(CombatSource b_CombatSource)
                {
                    var mCurrentTemp = b_BattleComponent.Parent.GetFindTheWay2D(b_CombatSource);

                    bool xAxial = mCurrentTemp.X == (int)mSelfPos.x;
                    bool yAxial = mCurrentTemp.Y == (int)mSelfPos.y;
                    // 击退长度
                    if (mConfig.OtherDataDic.TryGetValue(4, out int mRepelValue) && mRepelValue > 0)
                    {
                        ...
                        {
                            int mRepelValuex = xAxial ? 0 : (mCurrentTemp.X > mSelfPos.x ? mRepelValue : mRepelValue * -1);
                            int mRepelValuey = yAxial ? 0 : (mCurrentTemp.Y > mSelfPos.y ? mRepelValue : mRepelValue * -1);

                            mRandomResult = b_BattleComponent.Parent.GetFindTheWay2D(mCurrentTemp.X + mRepelValuex, mCurrentTemp.Y + mRepelValuey);
                        }
                        ...
                        switch (b_CombatSource.Identity)
                        {
                            case E_Identity.Hero:
                                {
                                    var mBeAttackerGamePlayer = b_CombatSource as GamePlayer;
                                    // 公告移动信息
                                    b_BattleComponent.Parent.MoveSendNotice(mCurrentTemp, mRandomResult, mBeAttackerGamePlayer, false);
                                    ...
                                    mWriteDataComponent.Save(b_CombatSource.UnitData, dBProxy2).Coroutine();
                                }
                                break;
                            case E_Identity.Enemy:
                                {
                                    if ((b_CombatSource as Enemy).IsCheckState(3) == false)
                                        // 公告移动信息
                                        b_BattleComponent.Parent.MoveSendNotice(mCurrentTemp, mRandomResult, b_CombatSource as Enemy, false);
                                }
                                break;
                            case E_Identity.Npc:
                                break;
                            case E_Identity.Pet:
                                {
                                    // 公告移动信息
                                    b_BattleComponent.Parent.MoveSendNotice(mCurrentTemp, mRandomResult, b_CombatSource as Pets, false);
                                }
                                break;
                            case E_Identity.Summoned:
                                {
                                    // 公告移动信息
                                    b_BattleComponent.Parent.MoveSendNotice(mCurrentTemp, mRandomResult, b_CombatSource as Summoned, false);
                                }
                                break;
                            default:
                                break;
