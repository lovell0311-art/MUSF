...
namespace ETHotfix
{
    ...
    /// 祝福
    /// </summary>
    [HeroSkillAttribute(BindId = (int)E_HeroSkillId.ZhuFu224)]
    public partial class C_HeroSkill_ZhuFu224 : C_HeroSkillSource
    {
    ...
    /// 祝福
    /// </summary>
    public partial class C_HeroSkill_ZhuFu224
    {
        ...
        public override bool UseSkill(CombatSource b_Attacker, CombatSource b_BeAttacker, BattleComponent b_BattleComponent)
        {
            ...
            Action<long, long, long> mSyncAction = (b_CombatRoundId, b_AttackerId, b_BeAttackerId) =>
            {
                ...
                int mStrengthValue = mWillpower / 100;
                if (mStrengthValue <= 0) mStrengthValue = 1;

                if (b_Attacker.BattleMasteryDic.TryGetValue(E_BattleMasteryState.Skill_ZuFuStrengthen_250, out var mMasteryValue))
                {
                ...
                Func<CombatSource.BattleSyncTimerTask> mCreateFunc = () =>
                {
                    ...
                    mBattleSyncTimer.DisposeAction = (b_CombatSource, b_BattleComponent, b_TimerTask) =>
                    {
                        ...
                        b_CombatSource.RemoveHealthState(E_BattleSkillStats.ZhuFu224, b_BattleComponent);
                        b_CombatSource.UpdateHealthState();

                        if (b_CombatSource.Identity == E_Identity.Hero) (b_CombatSource as GamePlayer).DataUpdateProperty();
                    };
                    mBattleSyncTimer.SyncAction = (b_CombatSource, b_BattleComponent, b_TimerTask, b_TimeTick) =>
                    {
                        ...
                        if (b_CombatSource.HealthStatsDic.TryGetValue(E_BattleSkillStats.ZhuFu224, out var hp_Curse) == false)
                        {
                ...
                void action(GamePlayer b_BeAttacker)
                {
                    if (b_BeAttacker == null) return;

                    // 提高攻击力
                    b_BeAttacker.AddHealthState(E_BattleSkillStats.ZhuFu224, mStrengthValue, mConfig.PersistentTime, 0, mCreateFunc, b_BattleComponent);
                    b_BeAttacker.UpdateHealthState();

                    b_BeAttacker.DataUpdateProperty();
                }

                action(b_Attacker as GamePlayer);
