...
namespace ETHotfix
{
    ...
    /// 袭风刺
    /// </summary>
    [HeroSkillAttribute(BindId = (int)E_HeroSkillId.XiFengCi)]
    public partial class C_HeroSkill_XiFengCi116 : C_HeroSkillSource
    {
    ...
    /// 袭风刺
    /// </summary>
    public partial class C_HeroSkill_XiFengCi116
    {
        ...
        public override bool TryUse(CombatSource b_Attacker, CombatSource b_BeAttacker, C_FindTheWay2D b_Cell, BattleComponent b_BattleComponent, IActorResponse b_Response)
        {
            ...
            // 大师技能
            if (b_Attacker.BattleMasteryDic.ContainsKey(E_BattleMasteryState.Skill_XiFengCi129))
            {
                mDistance = 40;
            }
        ...
        public override bool UseSkill(CombatSource b_Attacker, CombatSource b_BeAttacker, BattleComponent b_BattleComponent)
        {
            ...
            Action<long, long, long> mSyncAction = (b_CombatRoundId, b_AttackerId, b_BeAttackerId) =>
            {
                ...
                if (b_Attacker.UnitData.X == b_Cell.X || b_Attacker.UnitData.Y == b_Cell.Y)
                {
                    ...
                    int mRadius = (int)Math.Ceiling(mDistance * 0.5f);
                    (int X, int Y) mStartPos = (mCenter.X - mRadius, mCenter.Y - mRadius);

                    int mDiameter = mDistance * 2;
                ...
                int GetHurtValue(E_GameProperty b_SpecialAttack)
                {
                    ...
                    if (b_Attacker.BattleMasteryDic.ContainsKey(E_BattleMasteryState.Skill_XiFengCi129))
                    {
                        int mMasteryValue = b_Attacker.BattleMasteryDic[E_BattleMasteryState.Skill_XiFengCi129];
                        mHurtValue += mMasteryValue;
                    }
                    if (b_Attacker.BattleMasteryDic.ContainsKey(E_BattleMasteryState.Skill_XiFengCi_skilled132))
                    {
                        int mMasteryValue = b_Attacker.BattleMasteryDic[E_BattleMasteryState.Skill_XiFengCi_skilled132];
                        mHurtValue += mMasteryValue;
                    }
