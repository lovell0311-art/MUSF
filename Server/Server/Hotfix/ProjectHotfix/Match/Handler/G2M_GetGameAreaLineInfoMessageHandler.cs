...
namespace ETHotfix
{
    ...
    public class G2M_GetGameAreaLineInfoMessageHandler : AMRpcHandler<G2M_GetGameAreaLineInfoMessage, M2G_GetGameAreaLineInfoMessage>
    {
        protected override async Task<bool> CodeAsync(Session session, G2M_GetGameAreaLineInfoMessage b_Request, M2G_GetGameAreaLineInfoMessage b_Response, Action<IMessage> b_Reply)
        {
            ...
            //if (len > mResultData.Count) len = mResultData.Count;
            //int i = len - 20;
            //if (i < 0) i = 0;
            //for (; i < len; i++)
            //{
            //    C_GameAreaInfo mDBGameAreaInfo = mResultData[i];
            //    Console.WriteLine($"Area:{b_Request.AreaId}i:{i}len:{len}");
            //    if (mDBGameAreaInfo.RealLine == 0) continue;
            //    if (b_Request.AreaId != mDBGameAreaInfo.GameAreaId) continue;
            ...
            //}
            foreach (var mDBGameAreaInfo in mResultData)
            {
                if (mDBGameAreaInfo.RealLine == 0) continue;
                if (b_Request.AreaId != mDBGameAreaInfo.GameAreaId) continue;
        ...
        private G2C_GameAreaInfoMessage2 GetMessage(C_GameAreaInfo b_DBGameAreaInfo)
        {
            ...
            mResult.GameAreaId = b_DBGameAreaInfo.RealLine;
