...
namespace ETModel
{
    ...
    public struct IdStruct
    {
        ...
        public IdStruct(long id)
        {
            ...
            result >>= 16;
            this.Process = (int)(result & IdGeneraterNew.Mask18bit);
            result >>= 18;
    ...
    public struct InstanceIdStruct
    {
        ...
        {
            ulong result = (ulong)id;
            this.Value = (uint)(result & IdGeneraterNew.Mask18bit);
            result >>= 18;
            this.Process = (int)(result & IdGeneraterNew.Mask18bit);
            result >>= 18;
            this.Time = (uint)result;
    ...
    public struct UnitIdStruct
    {
        ...
        {
            int v = (int)(((ulong)unitId >> 24) & 0x03ff); // 取出10bit
            return v;
        }
    }

    public class IdGeneraterNew : IDisposable
    {
        public const int Mask18bit = 0x03ffff;
        public static IdGeneraterNew Instance = new IdGeneraterNew();
        ...
        /// <summary>
        /// 获取 现在到 2020 年的时间戳 单位:秒
        /// </summary>
        /// <returns></returns>
        private uint TimeSince2020()
        {
            uint a = (uint)((CustomFrameWork.TimeHelper.ClientNow() - this.epoch2020) / 1000);
            return a;
        }

        private uint TimeSinceThisYear()
        {
            uint a = (uint)((CustomFrameWork.TimeHelper.ClientNow() - this.epochThisYear) / 1000);
            return a;
        }
        ...
        public long GenerateInstanceId()
        {
            ...
            else
            {
                ...
                if (this.instanceIdValue > IdGeneraterNew.Mask18bit - 1) // 18bit
                {
            ...
            InstanceIdStruct instanceIdStruct = new InstanceIdStruct(this.lastInstanceIdTime, OptionComponent.Options.AppId, this.instanceIdValue);
        ...
        public long GenerateId()
        {
            ...
            IdStruct idStruct = new IdStruct(this.lastIdTime, OptionComponent.Options.AppId, value);
        ...
        public long GenerateUnitId(int zone)
        {
            ...
            UnitIdStruct unitIdStruct = new UnitIdStruct(zone, OptionComponent.Options.AppId, this.lastUnitIdTime, this.unitIdValue);
