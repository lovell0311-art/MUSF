...
namespace ETModel
{
    ...
    public sealed class TChannel : AChannel
    {
        ...
        private CircularBuffer sendBuffer = new CircularBuffer();
        ...
        public void TransferBufferTo(RTChannel channel)
        {
            (this.sendBuffer, channel.sendBuffer) = (channel.sendBuffer, this.sendBuffer);

            if(parser.state == ParserState.PacketBody)
            {
                // parser 中还存了 2/4bytes 数据
                switch (this.GetService().PacketSizeLength)
                {
                    case Packet.PacketSizeLength4:
                        if (parser.packetSize > ushort.MaxValue * 16 * 4)
                        {
                            throw new Exception($"send packet too large: {parser.packetSize}");
                        }
                        this.packetSizeCache.WriteTo(0, (int)parser.packetSize);
                        break;
                    case Packet.PacketSizeLength2:
                        if (parser.packetSize > ushort.MaxValue)
                        {
                            throw new Exception($"send packet too large: {parser.packetSize}");
                        }
                        this.packetSizeCache.WriteTo(0, (ushort)parser.packetSize);
                        break;
                    default:
                        throw new Exception("packet size must be 2 or 4!");
                }

                channel.recvBuffer.Write(this.packetSizeCache, 0, this.packetSizeCache.Length);
                parser.state = ParserState.PacketSize;
            }
            channel.recvBuffer.TransferFrom(this.recvBuffer);
        }

        public void TransferBufferFrom(RTChannel channel)
        {
            this.sendBuffer.TransferFrom(channel.sendBuffer);
            this.recvBuffer.TransferFrom(channel.recvBuffer);
        }
    }
}