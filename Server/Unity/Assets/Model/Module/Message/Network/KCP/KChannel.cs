...
namespace ETModel
{
	...
	public class KChannel : AChannel
	{
		...
		private void NormalizeRecvSegments(byte[] data, int offset, int length, uint conv)
		{
		...
		public void HandleRecv(byte[] date, int offset, int length)
		{
			...
			lock (this.kcpSyncRoot)
			{
				...
				this.NormalizeRecvSegments(date, offset, length, conv);
		...
		public override void Send(MemoryStream stream)
		{
			lock (this.kcpSyncRoot)
			{
				...
				unsafe
				{
					fixed (byte* pBytes = bytes, pBuffer = stream.GetBuffer())
					{
						Buffer.MemoryCopy(pBuffer + stream.Position, pBytes, size, size);
					}
				}
