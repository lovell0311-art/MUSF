...
namespace UnityEngine.UI
{
    ...
    public class Image : MaskableGraphic, ISerializationCallbackReceiver, ILayoutElement, ICanvasRaycastFilter
    {
        ...
        private static bool RequiresAtlasRegistration(Sprite sprite)
        {
            if (sprite == null)
            {
                return false;
            ...
            return sprite.texture == null || sprite.isUsingPlaceholder;
#else
            // Unity 2020 LTS does not expose placeholder sprite APIs.
            return sprite.texture == null;
#endif
        }

        private static bool ShouldKeepTracked(SpriteAtlas spriteAtlas)
        {
#if UNITY_2021_2_OR_NEWER
            return spriteAtlas != null && spriteAtlas.IsPlaceholder();
#else
            return false;
#endif
        }
        ...
        {
            if (RequiresAtlasRegistration(activeSprite))
            {
                TrackImage(this);
                m_Tracked = true;
            }
        ...
        static void RebuildImage(SpriteAtlas spriteAtlas)
        {
            for (var i = m_TrackedTexturelessImages.Count - 1; i >= 0; i--)
            {
                ...
                {
                    g.SetAllDirty();
                    if (!ShouldKeepTracked(spriteAtlas))
                    {
