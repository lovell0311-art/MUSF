using System.Collections.Generic;
using ETModel;

namespace ETHotfix
{
	[Config((int)(AppType.Game))]
	public partial class DropItem_SpecialConfigCategory : ACategory<DropItem_SpecialConfig>
	{
	}

	///<summary>特殊掉落物品 </summary>
	public class DropItem_SpecialConfig: IConfig
	{
		public long Id { get; set; }
		 ///<summary>掉落组id </summary>
		public int TreasureChestId;
		 ///<summary>物品id </summary>
		public int ItemId;
		 ///<summary>物品名 </summary>
		public string ItemName;
		 ///<summary>强化等级 </summary>
		public int Level;
		 ///<summary>数量 </summary>
		public int Quantity;
		 ///<summary>追加列表id </summary>
		public int OptListId;
		 ///<summary>追加等级 </summary>
		public int OptLevel;
		 ///<summary>拥有技能 </summary>
		public int HasSkill;
		 ///<summary>套装id </summary>
		public int SetId;
		 ///<summary>绑定物品 </summary>
		public int IsBind;
		 ///<summary>卓越属性 </summary>
		public List<int> OptionExcellent;
		 ///<summary>是任务物品 </summary>
		public int IsTaskItem;
		 ///<summary>自定义属性方法 </summary>
		public string CustomAttrMathod;
		 ///<summary>掉落率 </summary>
		public int DropRate;
	}
}
