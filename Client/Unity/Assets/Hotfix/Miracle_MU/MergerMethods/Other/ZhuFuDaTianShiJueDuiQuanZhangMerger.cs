...
namespace ETHotfix
{
    ...
    /// </summary>
    [MergerSystem(706)]
    public class ZhuFuDaTianShiJueDuiQuanZhangMerger : MergerMethod
    {
        public override bool CanUserThisMergerMethod()
        {
            ...
            Log.DebugGreen($"祝福大天使绝对权杖合成：{CheckItems.Count}");
            //标题
            AddTextTitle("祝福大天使绝对权杖合成");
            ///必须材料
            AddMustItemInfoText(isHave: MustItem.IsDaTianShiZhiJueDuiQuanRen = IsHaveYouLingZhanMa(out int lev, out int optLevel), str: $"大天使绝对权杖(+{lev})追{optLevel}\t\tx1");
            AddMustItemInfoText("大天使绝对权杖\t\tx1", MustItem.IsData = IsHaveItem(itemConfigId: 100005));
           ...
           // AddSubItemInfoText("保护符咒\t\tx1", IsHaveItem(itemConfigId: 320318));
            ...
            bool IsHaveYouLingZhanMa(out int lev, out int optLevel)
            {
                ...
                {
                    if (CheckItems[i].ConfigId == 100005)
                    {
