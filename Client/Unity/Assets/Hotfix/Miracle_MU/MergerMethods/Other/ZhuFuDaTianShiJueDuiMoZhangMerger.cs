...
namespace ETHotfix
{
    ...
    /// </summary>
    [MergerSystem(705)]
    public class ZhuFuDaTianShiJueDuiMoZhangMerger : MergerMethod
    {
        public override bool CanUserThisMergerMethod()
        {
            ...
            MaxSuccessRate = 50;
            FailedDelete = true;
            mergerMethodId = 10031;
            mergerMethod = "BlessingWandSynthesis";
            ...
            Log.DebugGreen($"祝福大天使绝对魔杖合成：{CheckItems.Count}");
            //标题
            AddTextTitle("祝福大天使绝对魔杖合成");
            ///必须材料
            AddMustItemInfoText(isHave: MustItem.IsDaTianShiZhiJueDuiQuanRen = IsHaveYouLingZhanMa(out int lev, out int optLevel), str: $"大天使绝对魔杖(+{lev})追{optLevel}\t\tx1");
            AddMustItemInfoText("大天使绝对魔杖\t\tx1", MustItem.IsData = IsHaveItem(itemConfigId: 80032));
          ...
          //  AddSubItemInfoText("保护符咒\t\tx1", IsHaveItem(itemConfigId: 320318));
            ...
            bool IsHaveYouLingZhanMa(out int lev, out int optLevel)
            {
                ...
                {
                    if (CheckItems[i].ConfigId == 80032)
                    {
