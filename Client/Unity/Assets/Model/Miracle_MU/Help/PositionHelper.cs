...
namespace ETModel
{
	public static class PositionHelper
	{
        ...
        public static bool IsEqual(Vector3 vector, Vector3 vector3)
        { 
         return vector3.x==vector.x&& vector3.z==vector.z;
        }

        public static Vector3 Vector3ToInt(this Vector3 self)
        {
            return new Vector3((int)self.x,0,(int)self.z);
        }
        public static Vector3 Vector2ToVector3Int(this Vector2 self)
        {
            return new Vector3((int)self.x, 0, (int)self.y);
        }
		...
		public static Quaternion GetVector3ToQuaternion(Vector3 source, Vector3 dire)
		{
            ...
            direction.y = 0;//2D 平面 不需要高度
            if(direction==Vector3.zero) return Quaternion.identity;
            return Quaternion.LookRotation(direction, Vector3.up);
		}
        public static Quaternion GetVector3ToQuaternion(AstarNode source, AstarNode dire)
		{
           
			Vector3 nowPos = AstarComponent.Instance.GetVectory3(source);
			Vector3 direction = (AstarComponent.Instance.GetVectory3(dire) - nowPos).normalized;
            direction.y = 0;//2D 平面 不需要高度
            return Quaternion.LookRotation(direction, Vector3.up);
		}
        ...
        public static float Distance2D(AstarNode u1, AstarNode u2)
        {
            Vector2 v1 = new Vector2(u1.x, u1.z);
            Vector2 v2 = new Vector2(u2.x, u2.z);
            return Vector2.Distance(v1, v2)*2;//一个为2米
        }
        ...
        /// <returns></returns>
        public static float Distance(AstarNode value1, AstarNode value2)
        {
            float num1 = value1.x - value2.x;
            float num2 = value1.z- value2.z;
            return (float)Math.Sqrt(num1 * (double)num1 + num2 * (double)num2);
        }
        public static float DistanceAstarNode(AstarNode u1, AstarNode u2)
        {
            Vector2 v1 = new Vector2(u1.x, u1.z);
            Vector2 v2 = new Vector2(u2.x, u2.z);
            return Vector2.Distance(v1, v2);//一个为2米
        }
